...
## <a name="remarks"></a>Hinweise  
 ...
 Standardmäßig sind interne C-Laufzeitblöcke (`_CRT_BLOCK`) nicht in den Speicherzustandsvorgängen enthalten. Die [_CrtSetDbgFlag](../../c-runtime-library/reference/crtsetdbgflag.md)-Funktion kann für Aktivierung des `_CRTDBG_CHECK_CRT_DF`-Bits von `_crtDbgFlag` verwendet werden, um diese Blöcke in die Erkennung von Speicherverlusten und andere Speicherzustandsvorgänge einzuschließen. Freigegebene Speicherblöcke (`_FREE_BLOCK`) führen nicht dazu, dass `_CrtMemDifference` TRUE zurückgibt.  
  
 Weitere Informationen über Heapzustandsfunktionen und die `_CrtMemState`-Struktur finden Sie unter [Berichtsfunktionen für den Heapzustand](/visualstudio/debugger/crt-debug-heap-details). Informationen darüber, wie Speicherblöcke in der Debugversion des Basisheaps zugeordnet, initialisiert und verwaltet werden, finden Sie unter [Details zum CRT-Debugheap](/visualstudio/debugger/crt-debug-heap-details).  
