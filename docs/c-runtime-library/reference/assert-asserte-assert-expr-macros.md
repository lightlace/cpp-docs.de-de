---
title: "_ASSERT-, _ASSERTE-, _ASSERT_EXPR-Makros | Microsoft Docs"
ms.custom: ""
ms.date: "11/04/2016"
ms.reviewer: ""
ms.suite: ""
...
---
# _ASSERT-, _ASSERTE-, _ASSERT_EXPR-Makros
...
## Hinweise  
 ...
 `_ASSERT_EXPR`, `_ASSERT` und `_ASSERTE` werten ihr `booleanExpression`\-Argument aus, und wenn das Ergebnis `false` \(0\) ist, geben sie eine Diagnosemeldung aus und rufen [\_CrtDbgReportW](../../c-runtime-library/reference/crtdbgreport-crtdbgreportw.md) auf, um einen Debugbericht zu generieren. Das `_ASSERT`\-Makro gibt eine einfache Diagnosemeldung aus, `_ASSERTE` nimmt eine Zeichenfolgendarstellung des Fehlerausdrucks in die Nachricht mit auf, und `_ASSERT_EXPR` schließt die `message`\-Zeichenfolge in die Diagnosemeldung ein. Diese Makros führen keine Aktion aus, wenn `booleanExpression` zu einem anderen Wert als null ausgewertet wird.  
...
```Output  
Verwenden Sie die Assert-Makros zum Auswerten des Ausdrucks p1 == p2. crt_ASSERT_macro.c(54) : Findet _ASSERT 'Ich bin p1' == 'Ich bin p2'? crt_ASSERT_macro.c(55): Assertionsfehler! crt_ASSERT_macro.c(58): _ASSERTE findet "Ich bin p1" == "Ich bin p2"? crt_ASSERT_macro.c(59) : Assertionsfehler: p1 == p2 'Ich bin p1' != 'Ich bin p2'  
```  
