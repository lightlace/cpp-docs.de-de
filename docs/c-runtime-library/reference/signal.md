...
|`SIGTERM`|Beendigungsanforderung|  
 ...
 Wenn `sig` keinem der oben genannten Werte entspricht, wird der Handler für ungültige Parameter aufgerufen, wie in [Parameter Validation (Parameterüberprüfung)](../../c-runtime-library/parameter-validation.md) definiert. Wenn die weitere Ausführung zugelassen wird, legt diese Funktion `errno` auf `EINVAL` fest und gibt `SIG_ERR` zurück.  
