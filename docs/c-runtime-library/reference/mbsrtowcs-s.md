...
Konvertieren einer Zeichenfolge mit Multibytezeichen im aktuellen Gebietsschema in die entsprechende Zeichenfolge mit Breitzeichen.  Eine Version von [mbsrtowcs](../../c-runtime-library/reference/mbsrtowcs.md) mit Sicherheitserweiterungen gemäß der Beschreibung unter [Sicherheitsfunktionen in der CRT](../../c-runtime-library/security-features-in-the-crt.md).  
...
#### Parameter  
 ...
 \[in\] `count`  
 Die maximale Anzahl von Breitzeichen, die im `wcstr`\-Puffer gespeichert werden können, wobei das abschließende Nullzeichen nicht eingeschlossen ist, oder [\_TRUNCATE](../../c-runtime-library/truncate.md).  
...
-   Die Anzahl der Breitzeichen, die im `wcstr`\-Puffer gespeichert sind, ist gleich `count`.  
 ...
 Wenn die Sequenzen, auf die von `mbstr` und `wcstr` gezeigt wird, überlappen, ist das Verhalten von `mbsrtowcs_s` nicht definiert.  `mbsrtowcs_s` wird von der LC\_TYPE\-Kategorie des aktuellen Gebietsschemas beeinflusst.  
