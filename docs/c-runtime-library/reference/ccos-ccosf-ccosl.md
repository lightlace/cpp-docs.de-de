...
## Hinweise  
 Da C\+\+ das Überladen zulässt, können Sie Überladungen von `ccos` aufrufen, die `_Fcomplex`\- und `_Lcomplex`\-Werte verwenden und zurückgeben. In einem C\-Programm `ccos` immer Double und gibt ein `_Dcomplex` Wert.  
