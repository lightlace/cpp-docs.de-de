...
## Hinweise  
 Die `time`\-Funktion gibt die Anzahl von Sekunden zurück, die entsprechend der Systemuhr seit Mitternacht \(00: 00:00 Uhr\), 1. Januar 1970, koordinierte Weltzeit \(Coordinated Universal Time, UTC\) verstrichen sind. Der Rückgabewert wird in dem Speicherort gespeichert, der durch `timer` angegeben ist. Dieser Parameter kann gleich `NULL` sein. Ist dies der Fall, wird der Rückgabewert nicht gespeichert.  
...
|-------------|---------------------------|  
|`time`, `_time32`, `_time64`|C: \<time.h\>, C\+\+: \<ctime\> oder \<time.h\>|  
