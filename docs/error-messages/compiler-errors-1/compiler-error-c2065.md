...
## <a name="example-precompiled-header-isnt-first"></a>Beispiel: vorkompilierter Header ist nicht erste

Dieser Fehler kann auftreten, wenn Sie alle Präprozessordirektiven, z. B. ablegen #include, #define oder #pragma, vor der #include eine vorkompilierte Headerdatei. Wenn die Quelldatei eine vorkompilierte Headerdatei verwendet (d. h., wenn sie mithilfe von kompiliert wird die **"/ Yu"** -Compileroption) und dann alle Präprozessordirektiven vor der vorkompilierten Headerdatei ignoriert werden.

In diesem Beispiel wird nicht kompiliert, da `cout` und `endl` definiert sind, der \<Iostream >-Header, der ignoriert wird, da es, bevor Sie die vorkompilierte Headerdatei enthalten ist. Um dieses Beispiel zu erstellen, erstellen Sie alle drei Dateien kompilieren Sie "stdafx.cpp", und dann kompilieren Sie C2065_pch.cpp.
...
Um dieses Problem zu beheben, fügen den #include \<Iostream > in der Datei der vorkompilierten Header, oder verschieben Sie ihn auch nachdem die vorkompilierte Headerdatei in der Quelldatei enthalten ist.
