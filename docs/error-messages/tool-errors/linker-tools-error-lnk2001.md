...
[Kompilieren Sie mischen Inline- und nicht Optionen](../../error-messages/tool-errors/function-inlining-problems.md) auf verschiedenen Modulen können LNK2001. Wenn eine C++-Bibliothek mit Inlinefunktionen eingeschaltet erstellt wird (**/Ob1** oder **/Ob2**) jedoch die entsprechende Headerdatei, beschreibt die Funktionen inlining deaktiviert (kein `inline` Schlüsselwort), dieser Fehler tritt auf. Um dieses Problem zu beheben, definieren Sie die Funktionen `inline` in der Headerdatei, die Sie in anderen Quelldateien enthalten.  
