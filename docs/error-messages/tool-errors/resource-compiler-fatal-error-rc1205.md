---
title: 'Ressourcencompiler: Schwerwiegender Fehler RC1205'
ms.date: 11/04/2016
f1_keywords:
- RC1205
helpviewer_keywords:
- RC1205
ms.assetid: 6cfbe019-ad8a-4267-b4b9-0d675bb3819a
ms.openlocfilehash: 7593c6ca0ab12ef21297a592b866eb6a87cef8fe
ms.sourcegitcommit: 0ab61bc3d2b6cfbd52a16c6ab2b97a8ea1864f12
ms.translationtype: MT
ms.contentlocale: de-DE
ms.lasthandoff: 04/23/2019
ms.locfileid: "62359064"
---
# <a name="resource-compiler-fatal-error-rc1205"></a>Ressourcencompiler: Schwerwiegender Fehler RC1205

Ungültige Codepage

Das Angeben der Codepage (/ C) folgte Option eine ungültige Codepage. Weitere Informationen hierzu finden Sie unter [Codepages](../../c-runtime-library/code-pages.md) in der *Laufzeitbibliotheksreferenz* .