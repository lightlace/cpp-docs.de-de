---
title: Compiler-Fehler C3631 | Microsoft-Dokumentation
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
ms.technology:
- devlang-cpp
ms.tgt_pltfrm: 
ms.topic: error-reference
f1_keywords:
- C3631
dev_langs:
- C++
helpviewer_keywords:
- C3631
ms.assetid: 88cbd2d5-6fef-4940-be34-d8cbe816d3da
caps.latest.revision: 11
author: corob-msft
ms.author: corob
manager: ghogen
translation.priority.ht:
- cs-cz
- de-de
- es-es
- fr-fr
- it-it
- ja-jp
- ko-kr
- pl-pl
- pt-br
- ru-ru
- tr-tr
- zh-cn
- zh-tw
translationtype: Machine Translation
ms.sourcegitcommit: c243063a9770542f137d5950e8a269f771960f74
ms.openlocfilehash: 3ed04a7f8a7070471ea97fb583af386c7922f6f6
ms.lasthandoff: 02/24/2017

---
# <a name="compiler-error-c3631"></a>Compilerfehler C3631
„Funktion“: Verwaltete oder WinRT-Ereignisse können nicht überladen werden.  
  
 Ein verwaltetes oder WinRT-Ereignis kann nicht überladen werden.  
  
 C3631 ist nur mit der veralteten-Compileroption erreichbar **/CLR: oldSyntax**.  

