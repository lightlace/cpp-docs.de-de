...
1. Auf der **Datei-DSN** auf **hinzufügen**.  
  
1. In der **neue Datenquelle erstellen** im Dialogfeld Wählen Sie ein Excel-Treiber aus, und klicken Sie dann auf **Weiter**.  
  
1. Klicken Sie auf **Durchsuchen**, und wählen Sie den Namen der Datei, die als Datenquelle verwendet werden.  
...
1. In der **ODBC Microsoft Excel Setup** Dialogfeld wählen die Datenbankversion und der Arbeitsmappe.  
...
1. Rufen Sie die `OpenEx` oder `Open` Member-Funktion.  
...
1. Anstatt die Zerstörung des Objekts aufrufen seiner `OpenEx` oder `Open` Member-Funktion erneut aus.  
