...
1. Ändern Sie den Code in die `Execute` -Methode in der [CMyProviderRowset(MyProviderRS.h)](cmyproviderrowset-myproviderrs-h.md). Ein Beispiel finden Sie unter [Einlesen von Zeichenfolgen in den OLE DB-Anbieter](../../data/oledb/reading-strings-into-the-ole-db-provider.md).  
  
1. Bearbeiten Sie die Eigenschaft zugeordnet wird, im [MyProviderDS.h](cmyprovidersource-myproviderds-h.md), [MyProviderSess.h](cmyprovidersession-myprovidersess-h.md), und [MyProviderRS.h](cmyproviderrowset-myproviderrs-h.md). Der Assistent erstellt die eigenschaftenzuordnungen, die alle Eigenschaften enthalten, die ein Anbieter implementieren kann. Durchlaufen Sie die eigenschaftenzuordnungen, und entfernen oder kommentieren Sie die Eigenschaften, die Ihr Anbieter nicht unterstützen muss.  
  
1. Aktualisieren von Makro, das im befinden [CMyProviderRowset(MyProviderRS.h)](cmyproviderrowset-myproviderrs-h.md). Ein Beispiel finden Sie unter [Speichern von Zeichenfolgen im OLE DB-Anbieter](../../data/oledb/storing-strings-in-the-ole-db-provider.md).  
