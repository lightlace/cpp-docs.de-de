---
title: "Threading und Marshalling (C++/CX) | Microsoft Docs"
ms.custom: ""
ms.date: "12/30/2016"
...
f1_keywords: 
  - "C4451"
helpviewer_keywords: 
  - "Threadingprobleme, C++/CX"
  - "Agilität, C++/CX"
  - "C++/CX, Threadingprobleme"
...
In den meisten Fällen kann auf Instanzen von [!INCLUDE[wrt](../cppcx/includes/wrt-md.md)]\-Klassen, wie z. B. C\+\+\-Objekte, von jedem Thread aus zugegriffen werden. Solche Klassen werden als "agil" bezeichnet. Einige wenige der mit Windows ausgelieferten [!INCLUDE[wrt](../cppcx/includes/wrt-md.md)]\-Klassen ist allerdings nicht agil und muss daher ähnlich wie COM\-Objekte und nicht wie Standard\-C\+\+\-Objekte verarbeitet werden. Sie müssen zwar kein COM\-Experte sein, um nicht agile Klassen zu verwenden, aber das Threadmodell dieser Klassen sowie ihr Marshallingverhalten beachten. Dieser Artikel ist ein Leitfaden für die seltenen Fälle, in denen Sie die Instanz einer nicht agilen Klasse verarbeiten müssen.  
  
## Threadmodell und Marshallingverhalten  
 Eine [!INCLUDE[wrt](../cppcx/includes/wrt-md.md)]\-Klasse kann gleichzeitigen Threadzugriff auf verschiedene Arten unterstützen, wie durch zwei Attribute angegeben wird, die darauf angewendet werden:  
...
## Nutzung von [!INCLUDE[wrt](../cppcx/includes/wrt-md.md)]\-Komponenten  
 Wenn Sie eine [!INCLUDE[win8_appname_long](../cppcx/includes/win8-appname-long-md.md)]\-App erstellen, interagieren Sie möglicherweise sowohl mit agilen als auch mit nicht agilen Komponenten. Wenn Sie mit nicht agilen Komponenten interagieren, wird möglicherweise die folgende Warnung angezeigt.  
  
### Compilerwarnung bei der Nutzung von nicht agilen Klassen \(C4451\)  
...
-   Die nicht agile Variable wird im Klassengültigkeitsbereich deklariert, und es besteht die Möglichkeit, dass verwendeter Code den Zeiger einschmuggelt, d. h. ihn in einem anderen Apartment ohne das richtige Marshalling verwendet.  
 ...
 Ist keine dieser Bedingungen zutreffend, können Sie die enthaltende Klasse als nicht agil markieren. Mit anderen Worten, Sie sollten nicht agile Objekte direkt nur in nicht agilen Klassen halten und nicht agile Objekte über Platform::Agile\<T\> in agilen Klassen.  
...
```  
 ...
 Wenn Sie in Visual C\+\+ einen Verweis auf eine prozessinterne [!INCLUDE[wrt](../cppcx/includes/wrt-md.md)]\-Klasse erstellen, die ein Marshallingverhalten von "None" hat, gibt der Compiler die Warnung C4451 aus, schlägt jedoch nicht vor, dass Sie die Verwendung von `Platform::Agile<T>` in Erwägung ziehen.  Der Compiler kann über diese Warnung hinaus keine Hilfe anbieten, sodass es in Ihrer Verantwortung liegt, die Klasse richtig zu verwenden und sicherzustellen, dass der Code STA\-Komponenten nur aus dem Benutzeroberflächenthread und MTA\-Komponenten nur aus einem Hintergrundthread aufruft.  
  
## Erstellen von agilen [!INCLUDE[wrt](../cppcx/includes/wrt-md.md)]\-Komponenten  
 Wenn Sie eine Verweisklasse in [!INCLUDE[cppwrt_short](../cppcx/includes/cppwrt-short-md.md)] definieren, ist sie standardmäßig agil – sie hat `ThreadingModel`\=Both und `MarshallingType`\=Agile.  Wenn Sie [!INCLUDE[cppwrl](../cppcx/includes/cppwrl-md.md)] verwenden, können Sie die Klasse agil machen, indem Sie von `FtmBase` ableiten, das `FreeThreadedMarshaller` verwendet.  Wenn Sie eine Klasse erstellen, die `ThreadingModel`\=Both oder `ThreadingModel`\=MTA hat, überprüfen Sie, ob die Klasse threadsicher ist. Weitere Informationen finden Sie unter [Erstellen und Verwenden von Objekten \(WRL\)](http://msdn.microsoft.com/de-de/d5e42216-e888-4f1f-865a-b5ccd0def73e).  
 ...
 Das folgende Beispiel veranschaulicht, wie `MarshalingBehavior` und `ThreadingModel` auf eine Laufzeitklasse in einer [!INCLUDE[wrt](../cppcx/includes/wrt-md.md)]\-Klassenbibliothek angewendet werden. Wenn eine App die DLL verwendet und das Schlüsselwort `ref new` benutzt, um ein `MySTAClass`\-Klassenobjekt zu aktivieren, ist das Objekt in einem Singlethread\-Apartment aktiviert und unterstützt kein Marshalling.  
