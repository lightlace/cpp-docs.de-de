...
Für [!INCLUDE[wrt](../cppcx/includes/wrt-md.md)]\-Typen gibt es vier verschiedene Umwandlungsoperatoren: [static\_cast](../cpp/static-cast-operator.md), [dynamic\_cast](../cpp/dynamic-cast-operator.md), [safe\_cast](../Topic/safe_cast%20\(C++%20Component%20Extensions\).md) und [reinterpret\_cast](../cpp/reinterpret-cast-operator.md).`safe_cast` und `static_cast` lösen eine Ausnahme aus, wenn die Umwandlung nicht ausgeführt werden kann; der Operator [static\_cast](../cpp/static-cast-operator.md) führt außerdem bei der Kompilierung eine Typprüfung durch.`dynamic_cast` gibt `nullptr` zurück, wenn der Typ nicht umgewandelt werden kann. Obwohl `reinterpret_cast` einen Wert ungleich Null zurückgibt, ist er möglicherweise ungültig. Aus diesem Grund wird empfohlen, dass Sie `reinterpret_cast` nur verwenden, wenn Sie wissen, dass die Umwandlung folgt. Außerdem empfehlen wir, im [!INCLUDE[cppwrt](../cppcx/includes/cppwrt-md.md)]\-Code \([!INCLUDE[cppwrt_short](../cppcx/includes/cppwrt-short-md.md)]\) keine Umwandlungen im C\-Format zu verwenden, da sie mit `reinterpret_cast` identisch sind.  
