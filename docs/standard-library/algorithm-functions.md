...
### <a name="remarks"></a>Hinweise  
 Die erste Vorlagenfunktion gibt den letzten Iterator `next` in `[``begin``,` `end``]` zurück, wobei `[``begin``, next)` ein Heap ist, der vom Funktionsobjekt `std::less<>` sortiert wird. Wenn der Abstand `end` `-` `begin` `< 2` ist, gibt die Funktion `end` zurück.  
...
### <a name="remarks"></a>Hinweise  
 Die erste Vorlagenfunktion gibt [is_sorted_until](http://msdn.microsoft.com/en-us/bbad99d0-deaa-4fe6-ae58-eb5b3e4dded0)`(` `first``,` `last``) ==` `last` zurück. Die Funktion operator< führt einen Reihenfolgenvergleich durch.  
