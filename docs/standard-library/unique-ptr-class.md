...
`Right` EIN `unique_ptr`.

`Nptr` Ein `rvalue` vom Typ `std::nullptr_t`.

`Ptr` EIN `pointer`.

`Deleter` Ein `deleter` -Funktion, die an gebunden ist eine `unique_ptr`.
...
`right` Ein `unique_ptr` zum Tauschen von Zeigern verwendet.
