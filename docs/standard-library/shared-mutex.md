...
Der Header &lt;shared_mutex > definiert die Klassen `shared_mutex` und `shared_timed_mutex`, die Vorlagen Klasse `shared_lock`und die Vorlagen Funktion `swap` für die gemeinsame Mutex-Unterstützung.
