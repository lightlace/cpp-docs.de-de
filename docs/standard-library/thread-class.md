...
`F` Eine anwendungsdefinierte Funktion der Thread ausgeführt werden soll.

`A` Eine Liste der zu übergebenden Argumente `F`.

`Other` Eine vorhandene `thread` Objekt.
