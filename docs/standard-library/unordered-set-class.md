...
### <a name="return-value"></a>Rückgabewert  
 Die Einzelelement-Memberfunktionen (1) und (2), Zurückgeben einer[Paar](../standard-library/pair-structure.md) , deren `bool` Komponente ist "true", wenn eine Einfügung erfolgte und "false", wenn das unordered_set-Element bereits ein Element enthalten, dessen Schlüssel einen entsprechenden Wert in der Reihenfolge aufweist. Die Iteratorkomponente des Rückgabewertpaars zeigt auf das neu eingefügten Element, wenn die `bool`-Komponente "true" lautet, oder auf das vorhandene Element, wenn die `bool`-Komponente "false" lautet.  
