...
Der Ausdruck `declval<To>() = declval<From>()` muss wohlgeformt sein, und es muss für den Compiler bekannt sein, dass er keine nicht trivialen Vorgänge benötigt. Sowohl `From` als `To` auch müssen komplette Typen, **void**oder Arrays mit unbekannter Grenze sein.
