...
## Hinweise  
 Die Operatorfunktion gibt `right` `<` `left` zurück.  Sie verwenden sie, um zu testen, ob `left` nach `right` sortiert wird, wenn die beiden Sätze verglichenes Element durch Element sind.  
