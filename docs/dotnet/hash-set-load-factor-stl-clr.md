...
## Hinweise  
 Die Memberfunktion wird `(float)`[hash\_set::size](../dotnet/hash-set-size-stl-clr.md)`() /` [hash\_set::bucket\_count](../dotnet/hash-set-bucket-count-stl-clr.md)`()` zurück.  Sie verwenden sie, um die durchschnittliche Bucketgröße zu bestimmen.  
