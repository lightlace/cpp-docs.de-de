...
|[copy (STL/CLR)](#copy)|Kopiert die Werte aus einem Quellbereich einem Zielbereich, vorwärts zu durchlaufen.|
|[copy_backward (STL/CLR)](#copy_backward)|Kopiert die Werte aus einem Quellbereich einem Zielbereich, rückwärts durchlaufen.|
...
|[equal_range (STL/CLR)](#equal_range)|Sucht eine geordnete Sequenz von Werten und gibt zwei Positionen, die eine Untersequenz der Werte zu trennen, die auf ein angegebenes Element gleich sind.|
...
|[find_end (STL/CLR)](#find_end)|Gibt den letzte Untersequenz in einem Bereich, der mit einer angegebenen Sequenz identisch ist.|
|[find_first_of (STL/CLR)](#find_first_of)|Durchsucht einen Bereich für das erste Vorkommen eines beliebigen einen bestimmten Bereich von Elementen an.|
|[find_if (STL/CLR)](#find_if)|Die Position des ersten Elements in einer Sequenz von Werten zurückgegeben, in dem das Element eine angegebene Bedingung erfüllt ist.|
...
|[generate_n (STL/CLR)](#generate_n)|Weist die Werte durch ein Funktionsobjekt mit einer angegebenen Anzahl der Elemente generiert.|
|[includes (STL/CLR)](#includes)|Testet, ob ein sortierter Bereich alle Elemente in einem zweiten sortierten Bereich enthält.|
|[inplace_merge (STL/CLR)](#inplace_merge)|Kombiniert die Elemente aus zwei aufeinander folgenden sortierten Bereichen in einem einzelnen Bereich sortiert.|
|[iter_swap (STL/CLR)](#iter_swap)|Tauscht zwei Werte aus, auf die durch ein Paar angegebener Iteratoren verwiesen wird.|
|[lexicographical_compare (STL/CLR)](#lexicographical_compare)|Vergleicht zwei Sequenzen elementweise, bestimmen, welche Sequenz die kleinere der beiden befindet.|
|[lower_bound (STL/CLR)](#lower_bound)|Sucht die Position des ersten Elements in eine geordnete Sequenz von Werten, die einen Wert größer als oder gleich einem angegebenen Wert verfügt.|
|[make_heap (STL/CLR)](#make_heap)|Konvertiert Elemente aus einem angegebenen Bereich in einen Heap, in dem das erste Element auf dem Heap am größten ist.|
|[max (STL/CLR)](#max))|Vergleicht zwei Objekte und gibt den größeren der beiden.|
|[max_element (STL/CLR)](#max_element)|Sucht nach dem größten Element in einer angegebenen Sequenz von Werten aus.|
|[merge (STL/CLR)](#merge))|Kombiniert alle Elemente aus zwei sortierten Quellbereichen in einen einzelnen, sortierten Zielbereich.|
|[min (STL/CLR)](#min)|Vergleicht zwei Objekte und gibt die kleinere der beiden zurück.|
|[min_element (STL/CLR)](#min_element)|Sucht das kleinste Element in einer bestimmten Reihenfolge der Werte an.|
|[mismatch (STL/CLR)](#mismatch)|Vergleicht zwei Bereiche elementweise und gibt die erste Position, in dem ein Unterschied auftritt.|
...
|[nth_element (STL/CLR)](#nth_element)|Partitionen eine Sequenz von Elementen und ermittelt die `n`th-Element der Sequenz, damit alle Elemente davor kleiner oder gleich ist, werden und alle Elemente, die folgen, größer als oder gleich sind werden.|
|[partial_sort (STL/CLR)](#partial_sort)|Ordnet eine angegebene Anzahl von kleineren Elementen in einem Bereich in der aufsteigenden Reihenfolge an.|
|[partial_sort_copy (STL/CLR)](#partial_sort_copy)|Kopiert Elemente aus einem Quellbereich in einen Zielbereich, dass die Elemente aus dem Quellbereich sortiert werden.|
|[partition (STL/CLR)](#partition)|Ordnet Elemente in einem Bereich an, dass die Elemente, die ein unäres Prädikat erfüllen, diese vorangestellt sein, auf denen er erfüllt.|
|[pop_heap (STL/CLR)](#pop_heap)|Das größte Element von der Vorderseite eines Heaps verschiebt, an das Ende und bildet dann einen neuen Heap aus den übrigen Elementen.|
|[prev_permutation (STL/CLR)](#prev_permutation)|Ordnet eine Sequenz von Elementen an, damit, dass die ursprüngliche Reihenfolge von der lexikografisch vorherigen höheren Permutation ersetzt wird, falls es vorhanden ist.|
...
|[random_shuffle (STL/CLR)](#random_shuffle)|Sortiert eine Sequenz von `N` Elemente in einem Bereich in eine der `N`! möglichen per Zufall ausgewählten Anordnungen neu.|
...
|[remove_copy (STL/CLR)](#remove_copy)|Kopiert Elemente aus einem Quellbereich einem Zielbereich mit dem Unterschied, dass keine Elemente eines angegebenen Werts kopiert werden, ohne die Reihenfolge der restlichen Elemente zu beeinträchtigen.|
|[remove_copy_if (STL/CLR)](#remove_copy_if)|Kopiert die Elemente aus einem Quellbereich einem Zielbereich, mit Ausnahme derjenigen, die ein Prädikat erfüllen, ohne die Reihenfolge der restlichen Elemente zu beeinträchtigen.|
...
|[replace (STL/CLR)](#replace)|Ersetzt die Elemente in einem Bereich, die einen angegebenen Wert durch einen neuen Wert zu entsprechen.|
|[replace_copy (STL/CLR)](#replace_copy)|Kopiert die Elemente aus einem Quellbereich einem Zielbereich, ersetzen die Elemente, die einem angegebenen Wert durch einen neuen Wert übereinstimmen.|
...
Diese Funktion verhält sich wie die C++-Standardbibliothek-Funktion `find`. Weitere Informationen finden Sie unter [finden](../standard-library/algorithm-functions.md#find).
...
Diese Funktion verhält sich wie die C++-Standardbibliothek-Funktion `find_if`. Weitere Informationen finden Sie unter [Find_if](../standard-library/algorithm-functions.md#find_if).
...
Diese Funktion verhält sich wie die C++ standardbibliotheksfunktion `inplace_merge` Weitere Informationen finden Sie unter [Inplace_merge](../standard-library/algorithm-functions.md#inplace_merge).
...
Sucht die Position des ersten Elements in einem sortierten Bereich, die einen Wert kleiner als oder gleich einem angegebenen Wert, wobei das Sortierkriterium durch ein binäres Prädikat angegeben werden kann.
...
Partitioniert einen Bereich von Elementen und ermittelt die `n`th-Element der Sequenz im Bereich sind damit, dass alle Elemente davor kleiner oder gleich sind und alle Elemente, die sie in der Sequenz folgen, größer als oder gleich sind.
...
Sortiert eine Sequenz von `N` Elemente in einem Bereich in eine der `N`! möglichen per Zufall ausgewählten Anordnungen neu.
...
Diese Funktion verhält sich wie die C++-Standardbibliothek-Funktion `sort`. Weitere Informationen finden Sie unter [Sortierreihenfolge](../mfc/reference/cmfclistctrl-class.md#sort).
