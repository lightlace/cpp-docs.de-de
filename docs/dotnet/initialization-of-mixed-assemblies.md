...
Vor Visual Studio 2005 kompilierte DLLs mit der **"/ CLR"** Compileroption konnten beim Laden nichtdeterministische Deadlocks; dieses Problem wurde aufgerufen, die gemischte DLL laden oder Ladeprogramm Problem. Fast der gesamte Nichtdeterminismus wurde aus dem Prozess des Ladens gemischter DLLs entfernt. Es verbleiben jedoch weiterhin ein paar Szenarien, bei denen die Loadersperre (deterministisch) auftreten kann.
...
-   Bei der Kompilierung für Itanium-Plattformen liegt ein Bug in der Implementierung aller Funktionszeiger vor.  Wenn im vorhergehenden Beispiel myObject_p lokal in „during_loaderlock()“ definiert wäre, könnte der Aufruf auch in eine verwaltete Implementierung aufgelöst werden.  
