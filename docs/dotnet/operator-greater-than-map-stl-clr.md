...
## Hinweise  
 Die Operatorfunktion gibt `right` `<` `left` zurück.  Sie verwenden sie, um zu testen, ob `left` nach `right` sortiert wird, wenn die zwei Zuordnungen verglichenes Element durch Element sind.  
