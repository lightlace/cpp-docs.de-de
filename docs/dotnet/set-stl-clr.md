...
|[set::difference_type (STL/CLR)](#difference_type)|Der Typ des Abstands zwischen zwei Elementen (möglicherweise mit Vorzeichen).|  
...
|[set::size_type (STL/CLR)](#size_type)|Der Typ eines Abstands zwischen zwei Elementen (negativ).|  
...
|[set::count (STL/CLR)](#count)|Zählt die Elemente, die einem angegebenen Schlüssel entsprechen.|  
...
|[set::lower_bound (STL/CLR)](#lower_bound)|Sucht den Anfang des Bereichs, die einem angegebenen Schlüssel entspricht.|  
...
|[set::upper_bound (STL/CLR)](#upper_bound)|Sucht nach Ende des Bereichs, die einem angegebenen Schlüssel entspricht.|  
|[set::value_comp (STL/CLR)](#value_comp)|Der Delegat für zwei Elementwerte wird kopiert.|  
...
|[operator== (set) (STL/CLR)](#op_eq)|Bestimmt, ob eine `set` -Objekt gleich einem anderen `set` Objekt.|  
|[operator> (set) (STL/CLR)](#op_gt)|Bestimmt, ob eine `set` -Quellobjekt ist größer als ein anderes `set` Objekt.|  
...
### <a name="remarks"></a>Hinweise  
 Die Memberfunktion gibt die Länge der gesteuerten Sequenz zurück. Sie können erkennen, die Anzahl der Elemente, die derzeit in der kontrollierten Sequenz. Wenn Sie von Interesse ist, ob die Sequenz ungleich Größe finden Sie unter hat, [Set:: Empty (STL/CLR)](../dotnet/set-empty-stl-clr.md)`()`.  
...
### <a name="remarks"></a>Hinweise  
 Die Memberfunktion bestimmt das letzte Element `X` in der kontrollierten Sequenz, die Sortierung `key`. Wenn kein solches Element vorhanden ist, oder wenn `X` ist das letzte Element in der kontrollierten Sequenz gibt [Set:: End (STL/CLR)](../dotnet/set-end-stl-clr.md)`()`; andernfalls wird einen Iterator, der das erste Element nach `X`. Sie können damit derzeit suchen das Ende einer Sequenz von Elementen in der kontrollierten Sequenz, die einen angegebenen Schlüssel entsprechen.  
