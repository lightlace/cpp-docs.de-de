...
`DoPrintPreview` im Hauptfenster der Anwendung werden ausgeblendet werden. Steuerleisten, z. B. die Statusleiste können beibehalten werden durch Angabe in der pState ->*DwStates* Member (Dies ist eine Bitmaske und die Bits für einzelne Steuerleisten von AFX_CONTROLBAR_MASK (AFX_IDW_MYBAR) definiert sind). Das Fenster pState ->*nIDMainPane* ist das Fenster, das automatisch reshown und ausgeblendet werden soll. `DoPrintPreview` wird eine Schaltflächenleiste wird dann für die Preview-Benutzeroberfläche erstellt werden. Wenn Fenster besondere Behandlung erforderlich ist, z. B. ein- oder ausblenden anderen Fenstern, die vor dem erfolgen soll `DoPrintPreview` aufgerufen wird.
