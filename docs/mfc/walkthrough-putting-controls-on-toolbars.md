---
title: 'Exemplarische Vorgehensweise: Einfügen von Steuerelementen auf der Symbolleiste | Microsoft-Dokumentation'
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-mfc
...
1.  Reservieren Sie eine Platzhalterressourcen-ID für die Schaltfläche in der übergeordneten Symbolleistenressource. Weitere Informationen dazu, wie Sie die Schaltflächen erstellen, indem Sie mit der Symbolleisten-Editor in Visual Studio finden Sie unter den [Symbolleisten-Editor](../windows/toolbar-editor.md) Thema.  
...
3.  Führen Sie in der Message-Handler, der die Nachricht AFX_WM_RESETTOOLBAR verarbeitet wird folgende Schritte aus:  
  
    1.  Erstellen Sie das Schaltflächensteuerelement mithilfe einer von `CMFCToolbarButton` abgeleiteten Klasse.  
  
    2.  Ersetzen Sie die platzhalterschaltfläche mithilfe durch das neue Steuerelement [CMFCToolBar::ReplaceButton](../mfc/reference/cmfctoolbar-class.md#replacebutton). Sie können das Schaltflächenobjekt auf dem Stapel erstellen, da `ReplaceButton` das Schaltflächenobjekt kopiert und die Kopie beibehält.  
...
5.  Der Konstruktor von `CMFCToolbarComboBoxButton` akzeptiert drei Parameter: die Befehls-ID der Schaltfläche, den Index des Schaltflächensymbols und das Format des Kombinationsfelds. Legen Sie diese Parameter wie folgt fest:  
    ...
    2.  Verwendung [CCommandManager::GetCmdImage](reference/internal-classes.md) mit `ID_EDIT_FIND` den Bildindex abgerufen.  
...
7.  Verwenden der [IMPLEMENT_SERIAL](../mfc/reference/run-time-object-model-services.md#implement_serial) Makro, um die Kombinationsschaltfläche persistent zu machen. Der Arbeitsbereichs-Manager lädt und speichert den Zustand der Schaltfläche automatisch in der Windows-Registrierung.  
  
8.  Implementieren Sie den `ID_EDIT_FIND_COMBO`-Handler in der Dokumentenansicht. Verwendung [CMFCToolBar::GetCommandButtons](../mfc/reference/cmfctoolbar-class.md#getcommandbuttons) mit `ID_EDIT_FIND_COMBO` abzurufenden alle **finden** -kombinationsfeldschaltflächen. Aufgrund der Anpassung können mehrere Kopien einer Schaltfläche mit derselben Befehls-ID vorhanden sein.  
  
9. In der ID_EDIT_FIND-Meldungshandler `OnFind`, verwenden Sie [CMFCToolBar::IsLastCommandFromButton](../mfc/reference/cmfctoolbar-class.md#islastcommandfrombutton) zu bestimmen, ob der Suchbefehl von gesendet wurde die **finden** Kombinationsfelds-Schaltfläche. Wenn dies der Fall ist, suchen Sie den Text, und fügen Sie dem Kombinationsfeld die Suchzeichenfolge hinzu.  
