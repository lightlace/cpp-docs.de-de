...
*pstrAgent*  
Ein Zeiger auf eine Zeichenfolge, die den Namen der Anwendung oder der verbundenen Entität, die Internet-Funktionen aufrufen (z. B. "Microsoft Internet-Browser") bezeichnet. Wenn *PstrAgent* ist **NULL** (Standard), das Framework Ruft die globale Funktion [AfxGetAppName](application-information-and-management.md#afxgetappname), womit eine Null-terminierte Zeichenfolge aus einer Anwendungsverzeichnis Der Name. Einige Protokolle verwenden diese Zeichenfolge zum Identifizieren Ihrer Anwendung mit dem Server.
...
- `INTERNET_OPEN_TYPE_PRECONFIG` Verbinden Sie mit vorkonfigurierten Einstellungen in der Registrierung. Dieser Art wird als Standard festgelegt. Legen Sie für die Verbindung über einen Proxy TIS *DwAccessType* mit diesem Wert; Sie dann die Registrierung entsprechend festgelegt.
...
- `ERROR_NO_MORE_ITEMS` Es gibt kein Cookie für die angegebene URL und alle übergeordneten Elemente.
...
*pstrURL*  
Ein Zeiger auf den Namen der URL gelesen werden soll. Nur URLs mit der Datei ab:, ftp:, Gopher:, oder http: werden unterstützt. Bestätigt, wenn *PstrURL* ist **NULL**.
...
- `INTERNET_FLAG_TRANSFER_BINARY` Übertragen Sie die Datei als Binärdatei aus.
...
- `INTERNET_OPEN_FLAG_USE_EXISTING_CONNECT` Wenn möglich, alle bestehenden Verbindungen mit dem Server neue Anforderungen von generierten wiederverwenden `OpenUrl` statt zu eine neue Sitzung für jede verbindungsanforderung erstellen.
...
Der Zeiger, `OpenURL` gibt hängt *PstrURL*des Typ des Diensts. Die nachfolgende Tabelle veranschaulicht die möglichen Zeiger `OpenURL` zurückgeben kann.
