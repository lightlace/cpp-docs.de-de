---
title: CMapPtrToPtr Klasse | Microsoft Docs
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-mfc
...
## <a name="members"></a>Member  
 Die Memberfunktionen von `CMapPtrToPtr` ähneln den Memberfunktionen der Klasse [CMapStringToOb](../../mfc/reference/cmapstringtoob-class.md). Aufgrund dieser Ähnlichkeit können Sie die `CMapStringToOb`-Referenzdokumentation für Memberfunktionsbesonderheiten verwenden. Immer dort, wo eine `CObject` -Zeiger als Funktionsparameter oder Rückgabewert, ersetzen Sie einen Zeiger auf **"void"**. Immer dort, wo eine `CString` oder ein **const** Zeiger auf `char` als Funktionsparameter oder als Rückgabewert, ersetzen Sie einen Zeiger auf **"void"**.  
...
## <a name="remarks"></a>Hinweise  
 `CMapPtrToPtr` enthält das `IMPLEMENT_DYNAMIC`-Makro zur Unterstützung von Laufzeittypenzugriff und zum Sichern in ein `CDumpContext`-Objekt. Wenn Sie eine Sicherung der einzelnen kartenelemente (Zeigerwerte) benötigen, müssen Sie die Tiefe des sicherungskontexts auf 1 oder größer festlegen.  
  
 Zeiger auf Zeiger Karten können nicht serialisiert werden.  
