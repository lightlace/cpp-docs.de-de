...
|[AfxOleInitModule](#afxoleinitmodule)|Stellt OLE-Unterstützung von einer regulären DLL, die dynamisch mit MFC verknüpft wird.|
|[AfxNetInitModule](#afxnetinitmodule)|Bietet MFC-Sockets in einer regulären DLL, die dynamisch mit MFC verknüpft wird.|
...
|[AfxTermExtensionModule]()#afxtermextensionmodule)|Können MFC Cleanup der Erweiterungs-DLL beim jeden Prozess aus der DLL wird getrennt.|


## <a name="afx_ext_class"></a>AFX_EXT_CLASS
[Erweiterungs-DLLs](../../build/extension-dlls.md) verwenden Sie das Makro **AFX_EXT_CLASS** zum Exportieren von Klassen, die ausführbaren Dateien, die mit dem Erweiterungs-DLL verknüpft das Makro verwenden, um Klassen zu importieren.  
   
### <a name="remarks"></a>Hinweise  
 Mit der **AFX_EXT_CLASS** -Makro, das den gleichen Header Datei(en) verwendet, um die Erweiterungs-DLL erstellen kann mit den ausführbaren Dateien, die mit der DLL verwendet werden.  
...
Für OLE-Unterstützung von einer regulären DLL, die dynamisch mit MFC verknüpft wird, rufen Sie diese Funktion in der regulären DLL `CWinApp::InitInstance` Funktion der MFC-OLE-DLL nicht initialisieren.  
...
Für MFC-Sockets in einer regulären DLL, die dynamisch mit MFC verknüpft wird unterstützen, fügen Sie einen Aufruf dieser Funktion in der regulären DLL **CWinApp:: InitInstance** Funktion, um die MFC-Sockets-DLL nicht initialisieren.  
...
### <a name="remarks"></a>Hinweise  
 Die MFC-Sockets-DLL ist eine Erweiterungs-DLL. in der Reihenfolge für eine Erweiterungs-DLL in wired Abrufen einer **CDynLinkLibrary** Kette, müssen sie erstellen eine **CDynLinkLibrary** Objekt im Kontext von jedem Modul, das sie verwenden möchten. `AfxNetInitModule`erstellt die **CDynLinkLibrary** Objekt im Kontext der regulären DLL, sodass es in wired Ruft die **CDynLinkLibrary** Kette der reguläre DLL-Objekt.  
...
Mit dieser Funktion wird in einer Erweiterungs DLL `DllMain` die DLL nicht initialisieren.  
...
### <a name="parameters"></a>Parameter  
 `state`  
 Ein Verweis auf die [AFX_EXTENSION_MODULE-Struktur](afx-extension-module-structure.md) Struktur, die den Status des Erweiterungs-DLL-Modul nach der Initialisierung enthalten soll. Dieser Status umfasst eine Kopie der Objekte der Common Language Runtime-Klasse, die von der Erweiterungs-DLL, im Rahmen des normalen statische Objektkonstruktion ausgeführt initialisiert wurden, bevor `DllMain` eingegeben wird.  
  
 `hModule`  
 Ein Handle für den Erweiterungs-DLL-Modul.  
   
### <a name="return-value"></a>Rückgabewert  
 **"True"** ist die Erweiterungs-DLL erfolgreich initialisiert wurde, andernfalls **"false"**.  
...
-   Erstellen einer **CDynLinkLibrary** Objekt, wenn die DLL exportieren [CRuntimeClass Struktur](cruntimeclass-structure.md) Objekte oder verfügt über eine eigene benutzerdefinierte Ressourcen.    
 Sie erreichen `AfxTermExtensionModule` zum Bereinigen von Erweiterungs-DLL, wenn jeder Prozess von der Erweiterungs-DLL trennt (das geschieht, wenn der Prozess beendet wird oder wenn die DLL entladen wird, als Ergebnis des ein `AfxFreeLibrary` aufrufen).     
...
Mit dieser Funktion können MFC bereinigen ermöglichen die Erweiterungs-DLL, wenn jeder Prozess von der DLL trennt (das geschieht, wenn der Prozess beendet wird oder wenn die DLL entladen wird, als Ergebnis des eine `AfxFreeLibrary` aufrufen).  
...
### <a name="parameters"></a>Parameter  
 `state`  
 Ein Verweis auf die [AFX_EXTENSION_MODULE](afx-extension-module-structure.md) -Struktur, die den Status des Erweiterungs-DLL-Modul enthält.  
  
 *Kugel*  
 Wenn **"true"**, bereinigt alle Erweiterungs-DLL-Module. Andernfalls Cleanup nur das aktuelle DLL-Modul.  
...
```
 ...
 Wenn die Anwendung lädt und Erweiterungs-DLLs dynamisch freigegeben, achten Sie darauf Aufrufen `AfxTermExtensionModule`. Da die meisten Erweiterungs-DLLs werden nicht dynamisch geladen (in der Regel verknüpft sind über ihre Importbibliotheken), den Aufruf von `AfxTermExtensionModule` ist in der Regel nicht erforderlich.  
