...
Weitere Informationen zur Ausnahmebehandlung im allgemeinen oder zu `CDaoException` Objekten finden Sie in den Artikeln [Ausnahmebehandlung (MFC)](../../mfc/exception-handling-in-mfc.md) und [Ausnahmen: Daten Bank](../../mfc/exceptions-database-exceptions.md)Ausnahmen. Der zweite Artikel enthält Beispielcode, der die Ausnahmebehandlung in DAO veranschaulicht.
...
1. Überprüfen Sie die aktuelle [CDaoErrorInfo](../../mfc/reference/cdaoerrorinfo-structure.md) -Struktur, `GetErrorInfo` die im [m_pErrorInfo](#m_perrorinfo) -Datenmember zurückgegeben wird. Seine Member stellen Informationen zum DAO-Fehler bereit.
...
Weitere Informationen zur Behandlung von Fehlern in den MFC-DAO-Klassen finden Sie [im Artikel Ausnahmen: Daten Bank](../../mfc/exceptions-database-exceptions.md)Ausnahmen.
...
`GetErrorInfo`speichert die Informationen im `m_pErrorInfo` Datenmember des Ausnahme Objekts. Eine kurze Beschreibung der zurückgegebenen Informationen finden Sie unter [m_pErrorInfo](#m_perrorinfo). Wenn Sie eine Ausnahme vom Typ `CDaoException` abfangen, die von MFC ausgelöst wird, wird der `m_pErrorInfo` Member bereits ausgefüllt. Wenn Sie DAO direkt aufzurufen, müssen Sie die Member-Funktion des Exception `GetErrorInfo` -Objekts selbst zum Auffüllen `m_pErrorInfo`aufzurufen. Eine ausführlichere Beschreibung finden Sie in der [CDaoErrorInfo](../../mfc/reference/cdaoerrorinfo-structure.md) -Struktur.

Weitere Informationen zu DAO-Ausnahmen und Beispielcode finden Sie im Artikel [Ausnahmen: Daten Bank](../../mfc/exceptions-database-exceptions.md)Ausnahmen.
...
- AFX_DAO_ERROR_DFX_BIND eine Adresse, die in einem DAO-Daten Satz Feld Austausch (DFX)-Funktions Aufrufwert verwendet wird, ist nicht vorhanden oder ungültig (die Adresse wurde nicht zum Binden von Daten verwendet). Möglicherweise haben Sie in einem DFX-Befehl eine ungültige Adresse übermittelt, oder die Adresse ist zwischen DFX-Vorgängen ungültig.

- AFX_DAO_ERROR_OBJECT_NOT_OPEN Sie haben versucht, ein Recordset zu öffnen, das auf einem QueryDef-oder einem Tabledef-Objekt basiert, das sich nicht in einem geöffneten Zustand befand.
