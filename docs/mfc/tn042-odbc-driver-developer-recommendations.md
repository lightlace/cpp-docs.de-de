...
## <a name="cdatabases"></a>CDatabases  
 ...
 `SQLGetInfo SQL_DATA_SOURCE_READ_ONLY` muss unterstützt werden. Wenn "Y" zurückgegeben wird, werden keine Update-Operationen für die Datenquelle ausgeführt werden.  
...
## <a name="executesql"></a>ExecuteSQL  
 Zusätzlich zum Zuordnen und befreien einen temporären **Befehls beschäftigt**, `ExecuteSQL` Aufrufe `SQLExecDirect`, `SQLFetch`, `SQLNumResultCol` und `SQLMoreResults`. `SQLCancel` kann aufgerufen werden, auf die **Befehls beschäftigt**.  
...
## <a name="crecordsets"></a>CRecordsets  
 ...
 `SQLSetParam` zum Binden der Parameterdaten umfassend verwendet wird und **DATA_AT_EXEC** Funktionalität.  
...
## <a name="forwardonly-cursors"></a>"ForwardOnly"-Cursor  
 Nur `SQLFetch` ist erforderlich, damit die `Move` Vorgänge. Beachten Sie, dass **"forwardOnly"** Cursor unterstützen keine Updates.  
  
## <a name="snapshot-cursors"></a>Momentaufnahmecursor  
 Hardwaresnapshot-Funktionalität erfordert `SQLExtendedFetch` unterstützen. Wie oben bereits erwähnt, wird der ODBC-Cursorbibliothek erkennen, wenn ein Treiber nicht unterstützt `SQLExtendedFetch`, und geben Sie die erforderliche Unterstützung selbst.  
