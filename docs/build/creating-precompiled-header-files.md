...
ms.locfileid: "57825318"
---
# <a name="precompiled-header-files"></a>Vorkompilierte Headerdateien

Bei der Erstellung eines neuen Projekts in Visual Studio eine *vorkompilierte Headerdatei* mit dem Namen "pch.h" wird dem Projekt hinzugefügt. (In früheren Versionen von Visual Studio wurde die Datei "stdafx.h" bezeichnet.) Der Zweck der Datei ist, um den Buildprozess zu beschleunigen. Stabile Headerdateien, z. B. standardbibliotheksheader wie z. B. `<vector>`, hier eingeschlossen werden sollen. Der vorkompilierte Header wird kompiliert, nur, wenn sie und alle darin enthaltenen Dateien geändert werden. Wenn Sie nur Änderungen in Ihrem Projektquellcode vornehmen, wird der Build-Kompilierung für den vorkompilierten Header überspringen. 

Die Compileroptionen für vorkompilierte Header sind [/y](reference/y-precompiled-headers.md). In den Projekt-Eigenschaftenseiten, befinden sich die Optionen unter **Konfigurationseigenschaften > C/C++-> vorkompilierte Header**. Sie können vorkompilierte Header nicht verwenden, und Sie können angeben, dass des Headers file Name sowie Name und Pfad der Ausgabedatei. 

## <a name="custom-precompiled-code"></a>Benutzerdefinierte vorkompilierter code

Für große Projekte, die viel Zeit zum Erstellen verwenden, sollten Sie Sie berücksichtigen, erstellen benutzerdefinierte vorkompilierte Dateien. Die Microsoft C- und C++-Compiler stellen Optionen für das Vorkompilieren von beliebigem C- oder C++-Code bereit, einschließlich Inlinecode. Mithilfe dieser leistungsstarken Funktion können Sie einen stabilen Codeabschnitt kompilieren, den Code im kompilierten Zustand in einer Datei speichern und bei nachfolgenden Kompilierungen den vorkompilierten Code mit dem noch in Entwicklung befindlichen Code kombinieren. So können nachfolgende Kompilierungen beschleunigt werden, da der bereits stabile Code nicht neu kompiliert werden muss.
...
Die Optionen für vorkompilierte Header sind ["/ Yc" (Erstellen vorkompilierter Headerdatei)](reference/yc-create-precompiled-header-file.md) und [/Yu (vorkompilierte Headerdatei verwenden)](reference/yu-use-precompiled-header-file.md). Verwendung **"/ Yc"** einen vorkompilierten Header erstellen. Bei Verwendung mit dem optionalen [Hdrstop](../preprocessor/hdrstop.md) Pragma **"/ Yc"** können Sie beide Headerdateien vorkompilieren und Quellcode. Wählen Sie **"/ Yu"** auf eine vorhandene vorkompilierte Headerdatei in der vorhandenen Kompilierung verwenden. Sie können auch **/fp** mit der **"/ Yc"** und **"/ Yu"** Optionen, um einen alternativen Namen für die vorkompilierte Headerdatei anzugeben.
...
Die ["/ Yu"](reference/yu-use-precompiled-header-file.md) -Compileroption können Sie angeben, welche PCH-Datei verwendet.
...
Weitere Informationen zu Makefiles finden Sie unter [NMAKE-Referenz](reference/nmake-reference.md). Siehe auch [MSVC-Compiler-Optionen](reference/compiler-options.md) und [MSVC-Linkeroptionen](reference/linker-options.md).
