...
ms.locfileid: "32376233"
---
# <a name="common-visual-c-arm-migration-issues"></a>Häufig auftretende ARM-Migrationsprobleme bei Visual C++

Wenn Sie Microsoft Visual C++ (MSVC) verwenden, kann der gleiche C++-Quellcode unterschiedliche Ergebnisse auf die ARM-Architektur führen, als auf X86 oder X64 Architekturen funktioniert.
...
*Ein nicht definiertes Verhalten* trifft, die nicht durch die C++-Standard definiert wird, und, der durch einen Vorgang, der keine angemessenen Ergebnis hat verursacht wird: z. B. einen Gleitkommawert in eine Ganzzahl ohne Vorzeichen zu konvertieren, oder Verschieben eines Werts von einer Anzahl von Positionen ist negativ oder überschreitet die Anzahl der Bits im sein höhergestufter Typ.

*Die Implementierung definiertes Verhalten* trifft, die die C++-Standard der Compiler-Anbieter erfordert, um zu definieren und zu dokumentieren. Ein Programm kann problemlos auf die Implementierung definiertes Verhalten beruhen, obwohl dies daher nicht portabel sein kann. Beispiele für die Implementierung definiertes Verhalten sind die Größen der integrierten Datentypen und ihre ausrichtungsanforderungen. Ein Beispiel für einen Vorgang, der durch die Implementierung definiertes Verhalten betroffen sein könnten greift auf die Variable Argumentliste.

*Ohne Angabe von Verhalten* trifft, die die C++-Standard nicht deterministische absichtlich verlässt. Obwohl das Verhalten als nicht deterministisch angesehen wird, werden bestimmte Aufrufe von nicht definiertes Verhalten durch die Implementierung des Compilers bestimmt. Allerdings besteht keine Notwendigkeit für eine Compiler-Anbieter festlegen, das das Ergebnis Netzwerkkonto oder einheitliches Verhalten zwischen vergleichbare Aufrufe zu gewährleisten und besteht keine Notwendigkeit für die Dokumentation. Ein Beispiel für nicht definiertes Verhalten ist die Reihenfolge, in der Teilausdrücke, darunter Argumente zu einem Funktionsaufruf ausgewertet werden.
...
### <a name="volatile-keyword-default-behavior"></a>Standardverhalten volatile-Schlüsselwort

Der Compiler MSVC unterstützt zwei verschiedene Interpretationen der der `volatile` Speicher-Qualifizierer, die Sie angeben können, mithilfe von Compiler-Switches. Die [/volatile:ms](../build/reference/volatile-volatile-keyword-interpretation.md) Switch wählt Microsoft volatile Semantik, die starke Sortierung zu gewährleisten wie beim traditionellen Modell für X86- und X64 aufgrund der starken Speichermodell in diesen Architekturen wurde erweitert. Die [/volatile:iso](../build/reference/volatile-volatile-keyword-interpretation.md) Switch wählt die strenge C++ volatile Standardsemantik, die starke Reihenfolge nicht garantiert.
...
In der X86- und X64-Architekturen, die Standardeinstellung ist **/volatile:ms** daran, dass ein Großteil der Software, die bereits für diese Architekturen erstellt wurde mithilfe von MSVC darauf benötigt. Wenn Sie X86- und X64 Programme kompilieren, können Sie angeben der **/volatile:iso** Switch, um zu verhindern, dass unnötige Abhängigkeit von der herkömmlichen volatile Semantik und Portabilität höher stufen.
