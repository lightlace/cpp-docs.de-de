---
title: "Verwenden von accelerator-Objekten und accelerator_view-Objekten | Microsoft Docs"
ms.custom: ""
ms.date: "11/04/2016"
ms.reviewer: ""
ms.suite: ""
...
> [!NOTE]
>  Eine der Zugriffstasten, die von `accelerator::get_all` zurückgegeben werden, ist die CPU-Zugriffstaste. Sie können auf der CPU-Zugriffstaste keinen Code ausführen. Um die CPU-Zugriffstaste herauszufiltern, vergleichen Sie den Wert von der [Device_path](../Topic/accelerator::device_path%20Data%20Member.md) -Eigenschaft der Zugriffstaste, die zurückgegebene `accelerator::get_all` mit dem Wert der [Accelerator:: cpu_accelerator](../Topic/accelerator::cpu_accelerator%20Data%20Member.md). Weitere Informationen finden Sie im Abschnitt "Spezielle Zugriffstasten" in diesem Artikel.  
  
## <a name="shared-memory"></a>Freigegebener Arbeitsspeicher  
 Freigegebener Arbeitsspeicher ist der Arbeitsspeicher, auf den sowohl die CPU als auch die Zugriffstaste zugreifen kann. Durch die Verwendung von freigegebenem Arbeitsspeicher entfällt bzw. reduziert sich der Mehraufwand zum Kopieren von Daten zwischen CPU und der Zugriffstaste erheblich. Obwohl der Arbeitsspeicher freigegeben wird, kann darauf nicht von CPU und der Zugriffstaste gleichzeitig zugegriffen werden. In diesem Fall kommt es zu einem nicht definierten Verhalten. Der Accelerator-Eigenschaft [Supports_cpu_shared_memory](../Topic/accelerator::supports_cpu_shared_memory%20Data%20Member.md) gibt `true` wenn die Zugriffstaste freigegebenen Arbeitsspeicher unterstützt und die [Default_cpu_access_type](../Topic/accelerator::default_cpu_access_type%20Data%20Member.md) Eigenschaft ruft standardmäßig [Access_type](../Topic/access_type%20Enumeration.md) für Arbeitsspeicher auf der `accelerator`– z. B. `array`s zugeordnet der `accelerator`, oder `array_view` Objekte zugegriffen wird, auf die `accelerator`.  
...
-   Sie können übergeben `accelerator_view` -Objekten, die Aufrufe an die [Parallel_for_each](../Topic/parallel_for_each%20Function%20\(C++%20AMP\).md) Methode.  
...
- [Accelerator:: direct3d_ref-Datenmember](../Topic/accelerator::direct3d_ref%20Data%20Member.md): Diese Singlethread-Zugriffstaste verwendet Software auf der CPU, um eine generische Grafikkarte zu emulieren. Sie wird standardmäßig für das Debuggen verwendet, ist jedoch in der Produktion nicht vorteilhaft, da sie langsamer ist als die Hardwarezugriffstasten. Außerdem ist sie nur im DirectX SDK und im Windows SDK verfügbar und wird mit großer Wahrscheinlichkeit nicht auf den Computern der Kunden installiert. Weitere Informationen finden Sie unter [GPU-Code Debuggen](../Topic/Debugging%20GPU%20Code.md).  
  
- [Accelerator:: direct3d_warp-Datenmember](../Topic/accelerator::direct3d_warp%20Data%20Member.md): dieser Accelerator bietet eine alternative Lösung zur Ausführung von C++ AMP-Codes auf Multikern-CPUs, die Streaming SIMD Extensions (SSE) verwenden.  
  
- [Accelerator:: cpu_accelerator-Datenmember](../Topic/accelerator::cpu_accelerator%20Data%20Member.md): Sie können diese Zugriffstaste zum Einrichten von staging-Arrays verwenden. Sie kann keinen C++ AMP-Code ausführen. Weitere Informationen finden Sie unter der [Staging-Arrays in C++ AMP](http://go.microsoft.com/fwlink/p/LinkId=248485) Beitrag im Blog zur parallelen Programmierung in systemeigenem Code.  
  
## <a name="interoperability"></a>Interoperabilität  
 Die C++ AMP-Laufzeit unterstützt Interoperabilität zwischen der `accelerator_view` -Klasse und die Direct3D [ID3D11Device-Schnittstelle](http://go.microsoft.com/fwlink/p/LinkId=248488). Die [Create_accelerator_view](../Topic/create_accelerator_view%20Function.md) -Methode akzeptiert ein `IUnknown` -Schnittstelle und gibt ein `accelerator_view` Objekt. Die [Get_device](assetId:///8194125e-8396-4d62-aa8a-65831dea8439) -Methode akzeptiert ein `accelerator_view` Objekt und gibt eine `IUknown` Schnittstelle.  
