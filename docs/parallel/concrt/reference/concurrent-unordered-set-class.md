...
Die `concurrent_unordered_set` Klasse ist ein parallelitätssicher Container, der steuert, eine variabler Länge Sequenz von Elementen vom Typ K. Die Sequenz wird dargestellt, auf eine Weise, die es parallelitätssichere ermöglicht anfügen, Elementzugriff, Iteratorzugriff und Iteratordurchläufe sind.  
...
|Name|Beschreibung|  
|----------|-----------------|  
|[Concurrent_unordered_set-Konstruktor](#ctor)|Überladen. Erstellt einen parallelen ungeordneten Satz.|  
...
|[Insert-Methode](#insert)|Überladen. Fügt dem `concurrent_unordered_set`-Objekt Elemente hinzu.|  
|[Key_eq-Methode](#key_eq)|Gibt das gespeicherte Gleichheitsvergleich-Funktionsobjekt zurück.|  
|[Swap-Methode](#swap)|Vertauscht den Inhalt von zwei `concurrent_unordered_set`-Objekten. Diese Methode ist nicht nebenläufigkeitssicher.|  
|[Unsafe_erase-Methode](#unsafe_erase)|Überladen. Entfernt Elemente aus der `concurrent_unordered_set` an angegebenen Positionen. Diese Methode ist nicht nebenläufigkeitssicher.|  
  
### <a name="public-operators"></a>Öffentliche Operatoren  
  
|Name|Beschreibung|  
|----------|-----------------|  
|[Operator =-Operator](#operator_eq)|Überladen. Weist den Inhalt eines anderen `concurrent_unordered_set`-Objekts diesem Objekt zu. Diese Methode ist nicht nebenläufigkeitssicher.|  
...
### <a name="parameters"></a>Parameter  
 ...
 `_Hasher`  
 Die Hashfunktion für diese ungeordneten Satz.  
...
### <a name="remarks"></a>Hinweise  
 ...
 Der erste Konstruktor gibt einen leeren ursprünglichen Satz an und explizit angibt, für die Anzahl der Buckets, Hash-Funktion, die Gleichheitsfunktion und der Zuweisung verwendet werden sollen.  
  
 Der zweite Konstruktor gibt eine Zuweisung für den ungeordneten Satz.  
 ...
 Der letzte Konstruktor gibt eine Verschiebung des gleichzeitigen ungeordneten Satz `_Uset`.  
...
### <a name="remarks"></a>Hinweise  
 Die erste Memberfunktion bestimmt, ob ein Element X in der Sequenz vorhanden ist, dessen Schlüssel hat, der entsprechende Reihenfolge `value`. Wenn nicht, es wird eine solche ein Element X erstellt und initialisiert sie mit `value`. Die Funktion bestimmt dann, den Iterator `where` X kennzeichnet. Wenn eine Einfügung aufgetreten ist, gibt die Funktion `std::pair(where, true)`. Andernfalls gibt es `std::pair(where, false)`.  
...
### <a name="remarks"></a>Hinweise  
 Nach dem Löschen alle vorhandenen Elemente in einen parallelen ungeordneten Satz `operator=` kopiert oder verschiebt den Inhalt des `_Uset` in gleichzeitigen ungeordneten Satz.  
...
### <a name="parameters"></a>Parameter  
 `_Uset`  
 Das `concurrent_unordered_set` Objekt zum austauschen.  
