...
Mit MSVC gibt es mehrere Möglichkeiten, mit mehreren Threads zu programmieren: Sie können/WinRT C++und die Windows-Runtime-Bibliothek, die MFC-Bibliothek (Microsoft Foundation Class) C++,/CLI und die .NET-Laufzeit oder die C-Lauf Zeit Bibliothek und die Win32-API verwenden. In diesem Artikel geht es um Multithreading in C. Beispielcode finden Sie unter [Sample Multithread Program in C](sample-multithread-c-program.md).
...
`_beginthread`und `_beginthreadex` ähneln der Funktion "-Funktion" in der Win32-API, haben [jedoch die folgenden](/windows/win32/api/processthreadsapi/nf-processthreadsapi-createthread) Unterschiede:
...
Die [_endthread](../c-runtime-library/reference/endthread-endthreadex.md) -Funktion beendet einen von `_beginthread` erstellten Thread `_endthreadex` (und auf ähnliche Weise einen von `_beginthreadex`erstellten Thread beendet). Threads werden automatisch beendet, wenn sie abgeschlossen sind. `_endthread` und `_endthreadex` sind zum bedingten Beenden aus einem Thread heraus sinnvoll. Ein für die Kommunikationsverarbeitung verantwortlicher Thread kann z. B. seine Aktivität einstellen, wenn die Steuerung des Kommunikationsanschlusses derzeit nicht möglich ist.
...
> [!NOTE]
> Eine ähnliche Erörterung in der MFC-Sicht finden [Sie unter Multithreading: Programmiertipps](multithreading-programming-tips.md) und [Multithreading: Verwendungszwecke der Synchronisierungs Klassen](multithreading-when-to-use-the-synchronization-classes.md)
...
Das erste Argument `_beginthread` des Aufrufes ist ein Zeiger auf `BounceProc` die Funktion, die die Threads ausführt. Das zweite Argument legt die Standardgröße des Stapel für den Thread fest. Das letzte Argument ist eine ID-Nummer, die an `BounceProc`das-Argument übermittelt wird. `BounceProc`verwendet die ID-Nummer, um einen Ausgangswert für den Zufallszahlengenerator zu verwenden und das Farb Attribut und das Anzeige Zeichen des Threads auszuwählen.

Threads, die Aufrufe an die C-Laufzeitbibliothek oder die Win32-API richten, müssen über ausreichend Stapelspeicher für die Bibliotheks- und API-Funktionen verfügen, die von ihnen aufgerufen werden. Die C `printf` -Funktion erfordert mehr als 500 Bytes Stapel Speicher, und beim Aufrufen von Win32-API-Routinen müssen 2 KB Stapel Speicher verfügbar sein.
