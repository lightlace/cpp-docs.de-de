---
title: Operator IMAGEREL | Microsoft Docs
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-masm
ms.topic: reference
f1_keywords:
- operator IMAGEREL
- IMAGEREL
dev_langs:
- C++
helpviewer_keywords:
- operator IMAGEREL
- IMAGEREL operator
ms.assetid: 5b5ea425-36f0-467c-9262-62c484b7fdb4
author: corob-msft
ms.author: corob
ms.workload:
- cplusplus
ms.openlocfilehash: db4c22c1e12ff2850c61cb1b0aea7fe89eca5ca4
ms.sourcegitcommit: dbca5fdd47249727df7dca77de5b20da57d0f544
ms.translationtype: MT
ms.contentlocale: de-DE
ms.lasthandoff: 04/28/2018
ms.locfileid: "32055111"
---
# <a name="operator-imagerel"></a>Operator IMAGEREL
Gibt das Bild relativen Offset `expression`.  
  
## <a name="syntax"></a>Syntax  
  
```  
IMAGEREL expression  
```  
  
## <a name="remarks"></a>Hinweise  
 Der resultierende Wert wird häufig als eine RVA oder Relative virtuelle Adresse bezeichnet.  
  
 IMAGEREL ist nur mit COFF-Objekt prädikatauswertung verfügbar.  
  
## <a name="see-also"></a>Siehe auch  
 [Operatoren – Referenz](../../assembler/masm/operators-reference.md)