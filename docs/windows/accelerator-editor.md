...
Eine Liste der vordefinierten Zugriffstasten, finden Sie unter [Zugriffstasten](../windows/predefined-accelerator-keys.md).
...
Die folgende sind zulässige Einträge für die **Modifizierer** Eigenschaft in der tastenkombinationstabelle:
   ...
   |**STRG**|Der Benutzer muss drücken **STRG** vor der **Schlüssel** Wert. Mit dem ASCII-Typ nicht gültig.|
   |**UMSCHALTTASTE**|Der Benutzer muss drücken **UMSCHALT** vor der **Schlüssel** Wert.|
   |**Strg + Alt +**|Der Benutzer muss drücken **STRG** und **Alt** vor der **Schlüssel** Wert. Mit dem ASCII-Typ nicht gültig.|
   |**STRG + UMSCHALT**|Der Benutzer muss drücken **STRG** und **UMSCHALT** vor der **Schlüssel** Wert. Mit dem ASCII-Typ nicht gültig.|
   |**ALT + UMSCHALT**|Der Benutzer muss drücken **Alt** und **UMSCHALT** vor der **Schlüssel** Wert. Mit dem ASCII-Typ nicht gültig.|
...
Die folgende sind zulässige Einträge für die **Schlüssel** Eigenschaft in der tastenkombinationstabelle:
   ...
   |Ein einzelnes Zeichen der Tastatur im Bereich von A - Z (nur Großbuchstaben), ein Caretzeichen (^) vorangestellt.<br/><br/>Z. B. ^ C.|Diese Option gibt den ASCII-Wert des Schlüssels aus, wenn es gedrückt wird, mit der **STRG** -Taste gedrückt gehalten.|
...
> [!TIP]
> Eine Verknüpfung zum Definieren einer Zugriffstaste ist ein oder mehrere Einträge in der tastenkombinationstabelle mit der rechten Maustaste auf. Wählen Sie **Nächste Taste** und drücken Sie dann eine der Tasten bzw. Tastenkombinationen auf der Tastatur.
>
> Die **Nächste Taste** Befehl steht auch auf die **bearbeiten** Menü.
...
- Wenn die **Typ** Eigenschaft **ASCII**, **Modifizierer** Eigenschaft ist möglicherweise nur `None` oder `Alt`, oder es kann eine Zugriffstaste, die die verwendet**STRG** Schlüssel (gemäß den Schlüssel mit dem vorherigen eine `^`).
...
So bearbeiten Sie in einer Zugriffstastentabelle
...
So finden Sie einen Eintrag in einer geöffneten Zugriffstastentabelle
...
So fügen Sie zu einer Zugriffstastentabelle einen Eintrag hinzu:
...
1. Typ der *Schlüssel* Sie verwenden möchten, verwenden Sie als Zugriffstaste, oder mit der rechten Maustaste, und wählen **Nächste Taste** aus dem Kontextmenü eine Tastenkombination festlegen oder wechseln Sie zum Menü **bearbeiten**  >  **Nächste Taste**.
...
> [!TIP]
> Eine Verknüpfung zu löschen ist, drücken die **löschen** Schlüssel.

So verschieben oder kopieren Sie Eintrags in einer Zugriffstastentabelle in eine andere Ressourcenskriptdatei:
