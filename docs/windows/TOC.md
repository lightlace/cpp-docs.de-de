# [Übersicht über Windows-Programmierung in C++](overview-of-windows-programming-in-cpp.md)
# [Windows-Desktopanwendungen in C++](desktop-applications-visual-cpp.md)
## [Windows-Konsolenanwendungen in C++](console-applications-in-visual-cpp.md)
### [Exemplarische Vorgehensweise: Erstellen eines Standard C++-Programms (C++)](walkthrough-creating-a-standard-cpp-program-cpp.md)
### [Erstellen einer Konsolenanwendung](creating-a-console-application.md)
### [Win32-Anwendungs-Assistent](win32-application-wizard.md)
#### [Anwendungseinstellungen, Win32-Projekt-Assistent](application-settings-win-32-project-wizard.md)
## [Exemplarische Vorgehensweise: Erstellen von Windows-Desktopanwendungen (C++)](walkthrough-creating-windows-desktop-applications-cpp.md)
## [Erstellen einer leeren Windows-Desktopanwendung](creating-an-empty-windows-desktop-application.md)
## [Hinzufügen von Dateien zu leeren Win32-Anwendungen](adding-files-to-an-empty-win32-applications.md)
## [Arbeiten mit Ressourcendateien](working-with-resource-files.md)
### [Ressourcendateien (Visual Studio)](resource-files-visual-studio.md)
#### [Vorgehensweise: Erstellen einer Ressourcenskriptdatei](how-to-create-a-resource-script-file.md)
#### [Vorgehensweise: Erstellen einer Ressource](how-to-create-a-resource.md)
#### [Vorgehensweise: Öffnen einer Ressourcenskriptdatei außerhalb eines Projekts (eigenständig)](how-to-open-a-resource-script-file-outside-of-a-project-standalone.md)
#### [Vorgehensweise: Öffnen einer Ressourcenskriptdatei im Textformat](how-to-open-a-resource-script-file-in-text-format.md)
#### [Vorgehensweise: Hinzufügen von MFC-Unterstützung zu Ressourcenskriptdateien](how-to-add-mfc-support-to-resource-script-files.md)
#### [Vorgehensweise: Einfügen von Ressourcen zur Kompilierungszeit](how-to-include-resources-at-compile-time.md)
#### [Vorgehensweise: Kopieren von Ressourcen](how-to-copy-resources.md)
##### [Vorgehensweise: Ändern der Sprache oder Bedingung einer Ressource während des Kopierens](how-to-change-the-language-or-condition-of-a-resource-while-copying.md)
#### [Vorgehensweise: Symbolsuche in Ressourcen](how-to-search-for-symbols-in-resources.md)
#### [Vorgehensweise: Verwenden von Ressourcenvorlagen](how-to-use-resource-templates.md)
#### [Vorgehensweise: Importieren und Exportieren von Ressourcen](how-to-import-and-export-resources.md)
#### [Editierbare Dateitypen für Ressourcen](editable-file-types-for-resources.md)
#### [Bearbeiten von verwalteten Ressourcendateien](editing-managed-resource-files.md)
#### [Von der Ressourcenbearbeitung betroffene Dateien](files-affected-by-resource-editing.md)
#### [Manifestressourcen](manifest-resources.md)
##### [Vorgehensweise: Öffnen einer Manifestressource](how-to-open-a-manifest-resource.md)
### [Symbole: Ressourcenbezeichner](symbols-resource-identifiers.md)
#### [Anzeigen von Ressourcensymbolen](viewing-resource-symbols.md)
#### [Erstellen neuer Symbole](creating-new-symbols.md)
#### [Ändern nicht zugewiesener Symbole](changing-unassigned-symbols.md)
#### [Löschen nicht zugewiesener Symbole](deleting-unassigned-symbols.md)
#### [Öffnen des Ressourcen-Editors für ein bestimmtes Symbol](opening-the-resource-editor-for-a-given-symbol.md)
#### [Ändern eines Symbols](changing-a-symbol-or-symbol-name-id.md)
#### [Einfügen gemeinsam genutzter (schreibgeschützter) oder berechneter Symbole](including-shared-read-only-or-calculated-symbols.md)
#### [Vordefinierte Symbol-IDs](predefined-symbol-ids.md)
##### [Vordefinierte ATL-Symbole](atl-predefined-symbols.md)
##### [Vordefinierte MFC-Symbole](mfc-predefined-symbols.md)
##### [Vordefinierte Win32-Symbole](win32-predefined-symbols.md)
### [Ressourcen-Editor](resource-editors.md)
#### [Anzeigen und Bearbeiten von Ressourcen in einem Ressourcen-Editor](viewing-and-editing-resources-in-a-resource-editor.md)
#### [Zugriffstasten-Editor](accelerator-editor.md)
##### [Festlegen von Eigenschaften für Zugriffstasten](setting-accelerator-properties.md)
##### [Bearbeiten von Zugriffstastentabellen](editing-accelerator-tables.md)
##### [Zugriffstasten](predefined-accelerator-keys.md)
#### [Binary Editor](binary-editor.md)
#### [Dialog-Editor](dialog-editor.md)
##### [Erstellen eines Dialogfelds](creating-a-new-dialog-box.md)
##### [Ein- oder Ausblenden der Symbolleiste des Dialog-Editors](showing-or-hiding-the-dialog-editor-toolbar.md)
##### [Umschalten zwischen Dialogfeld-Steuerelementen und Code](switching-between-dialog-box-controls-and-code.md)
##### [Steuerelemente in Dialogfeldern](controls-in-dialog-boxes.md)
###### [Hinzufügen, Bearbeiten oder Löschen von Steuerelementen](adding-editing-or-deleting-controls.md)
###### [Auswählen von Steuerelementen](selecting-controls.md)
###### [Festlegen der Größe von Steuerelementen](sizing-individual-controls.md)
###### [Hinzufügen von Werten zu einem Kombinationsfeld-Steuerelement](adding-values-to-a-combo-box-control.md)
###### [Anordnung von Steuerelementen in Dialogfeldern](arrangement-of-controls-on-dialog-boxes.md)
####### [Modi des Dialog-Editors (Führungslinien und Raster)](dialog-editor-states-guides-and-grids.md)
####### [Gruppieren von Optionsfeldern in einem Dialogfeld](grouping-radio-buttons-on-a-dialog-box.md)
####### [Ausrichten von Steuerelementen](aligning-groups-of-controls.md)
####### [Ändern der Aktivierreihenfolge von Steuerelementen](changing-the-tab-order-of-controls.md)
###### [Benutzerdefinierte Steuerelemente im Dialog-Editor](custom-controls-in-the-dialog-editor.md)
###### [Definieren von Tastenkombinationen](defining-mnemonics-access-keys.md)
###### [Entwerfen eines Dialogfelds](testing-a-dialog-box.md)
##### [Zugriffstasten für den Dialog-Editor](accelerator-keys-for-the-dialog-editor.md)
##### [Problembehandlung beim Dialog-Editor](troubleshooting-the-dialog-editor.md)
##### [Definieren von Membervariablen für Dialogfeld-Steuerelemente](defining-member-variables-for-dialog-controls.md)
#### [Bildbearbeitung für Symbole](image-editor-for-icons.md)
##### [Bearbeiten von grafischen Ressourcen (Bildbearbeitung für Symbole)](editing-graphical-resources-image-editor-for-icons.md)
###### [Erstellen eines Symbols oder anderen Bilds (Bildbearbeitung für Symbole)](creating-an-icon-or-other-image-image-editor-for-icons.md)
###### [Verwenden eines Zeichentools (Bildbearbeitung für Symbole)](using-a-drawing-tool-image-editor-for-icons.md)
###### [Zeichnen von Linien oder geschlossenen Körpern (Bildbearbeitung für Symbole)](drawing-lines-or-closed-figures-image-editor-for-icons.md)
###### [Markieren eines Bildbereichs (Bildbearbeitung für Symbole)](selecting-an-area-of-an-image-image-editor-for-icons.md)
###### [Bearbeiten von Bestandteilen eines Bilds (Bildbearbeitung für Symbole)](editing-parts-of-an-image-image-editor-for-icons.md)
###### [Kippen eines Bilds (Bildbearbeitung für Symbole)](flipping-an-image-image-editor-for-icons.md)
###### [Größenänderungen bei Bildern (Bildbearbeitung für Symbole)](resizing-an-image-image-editor-for-icons.md)
###### [Erstellen von benutzerdefinierten Pinseln (Bildbearbeitung für Symbole)](creating-a-custom-brush-image-editor-for-icons.md)
###### [Ändern von Bildeigenschaften (Bildbearbeitung für Symbole)](changing-image-properties-image-editor-for-icons.md)
###### [Speichern von Bitmaps als GIFs oder JPEGs (Bildbearbeitung für Symbole)](saving-bitmaps-as-gifs-or-jpegs-image-editor-for-icons.md)
###### [Konvertieren von Bildern in andere Formate (Bildbearbeitung für Symbole)](converting-an-image-from-one-format-to-another-image-editor-for-icons.md)
###### [Bearbeiten eines Bilds außerhalb eines Projekts (Bildbearbeitung für Symbole)](editing-an-image-outside-of-a-project-image-editor-for-icons.md)
###### [Bild (Menü) (Bildbearbeitung für Symbole)](image-menu-image-editor-for-icons.md)
####### [Rastereinstellungen (Dialogfeld) (Bildbearbeitung für Symbole)](grid-settings-dialog-box-image-editor-for-icons.md)
###### [Symbolleiste (Bildbearbeitung für Symbole)](toolbar-image-editor-for-icons.md)
###### [Fensterbereiche (Bildbearbeitung für Symbole)](window-panes-image-editor-for-icons.md)
##### [Arbeiten mit Farben (Bildbearbeitung für Symbole)](working-with-color-image-editor-for-icons.md)
###### [Auswählen von Vordergrund- oder Hintergrundfarben (Bildbearbeitung für Symbole)](selecting-foreground-or-background-colors-image-editor-for-icons.md)
###### [Füllen von begrenzten Bereichen eines Bilds mit einer Farbe (Bildbearbeitung für Symbole)](filling-a-bounded-area-of-an-image-with-a-color-image-editor-for-icons.md)
###### [Übertragen einer Farbe eines Bilds auf andere Elemente (Bildbearbeitung für Symbole)](picking-up-a-color-from-an-image-to-use-elsewhere-image-editor-for-icons.md)
###### [Auswählen eines transparenten oder deckenden Hintergrunds (Bildbearbeitung für Symbole)](choosing-a-transparent-or-opaque-background-image-editor-for-icons.md)
###### [Invertieren der Farben in einer Markierung (Bildbearbeitung für Symbole)](inverting-the-colors-in-a-selection-image-editor-for-icons.md)
###### [Anpassen oder Ändern von Farben (Bildbearbeitung für Symbole)](customizing-or-changing-colors-image-editor-for-icons.md)
###### [Speichern und Laden von verschiedenen Farbpaletten (Bildbearbeitung für Symbole)](saving-and-loading-different-color-palettes-image-editor-for-icons.md)
###### [Fenster „Farben“ (Bildbearbeitung für Symbole)](colors-window-image-editor-for-icons.md)
##### [Symbole und Cursor: Bildressourcen für Anzeigegeräte (Bildbearbeitung für Symbole)](icons-and-cursors-image-resources-for-display-devices-image-editor-for-icons.md)
###### [Erstellen eines Gerätebilds (Bildbearbeitung für Symbole)](creating-a-device-image-image-editor-for-icons.md)
###### [Hinzufügen eines Bilds für ein anderes Anzeigegerät (Bildbearbeitung für Symbole)](adding-an-image-for-a-different-display-device-image-editor-for-icons.md)
###### [Kopieren eines Gerätebilds (Bildbearbeitung für Symbole)](copying-a-device-image-image-editor-for-icons.md)
###### [Löschen eines Gerätebilds (Bildbearbeitung für Symbole)](deleting-a-device-image-image-editor-for-icons.md)
###### [Erstellen transparenter oder invertierter Bereiche in Gerätebildern (Bildbearbeitung für Symbole)](creating-transparent-or-inverse-regions-in-device-images.md)
###### [Verwenden der 256-Farben-Palette (Bildbearbeitung für Symbole)](creating-a-256-color-icon-or-cursor-image-editor-for-icons.md)
###### [Festlegen des Hotspots eines Cursors (Bildbearbeitung für Symbole)](setting-a-cursor-s-hot-spot-image-editor-for-icons.md)
##### [Zugriffstasten (Bildbearbeitung für Symbole)](accelerator-keys-image-editor-for-icons.md)
#### [Menü-Editor](menu-editor.md)
##### [Erstellen eines Menüs](creating-a-menu.md)
##### [Erstellen eines Untermenüs](creating-a-submenu.md)
##### [Erstellen von Popupmenüs](creating-pop-up-menus.md)
##### [Bearbeiten mehrerer Menüs oder Menübefehle](selecting-multiple-menus-or-menu-commands.md)
##### [Verknüpfen von Menübefehlen](associating-a-menu-command-with-an-accelerator-key.md)
##### [Menübefehlseigenschaften](menu-command-properties.md)
#### [Zeichenfolgen-Editor](string-editor.md)
#### [Symbolleisten-Editor](toolbar-editor.md)
##### [Erstellen neuer Symbolleisten](creating-new-toolbars.md)
##### [Konvertieren von Bitmaps in Symbolleisten](converting-bitmaps-to-toolbars.md)
##### [Erstellen, Verschieben und Bearbeiten von Schaltflächen der Symbolleiste](creating-moving-and-editing-toolbar-buttons.md)
##### [Erstellen einer QuickInfo für eine Symbolleisten-Schaltfläche](creating-a-tool-tip-for-a-toolbar-button.md)
#### [Versionsinfo-Editor](version-information-editor.md)
# [Active Template Library (ATL)](../atl/TOC.md)
# [Microsoft Foundation Classes (MFC)](../mfc/TOC.md)
# [Freigegebene ATL- und MFC-Klassen](../atl-mfc-shared/TOC.md)
# [.NET-Entwicklung mit C++/CLI](../dotnet/dotnet-programming-with-cpp-cli-visual-cpp.md)
# [Komponentenerweiterungen für .NET und UWP](component-extensions-for-runtime-platforms.md)
## [Nachverfolgungsverweisoperator (C++/CLI und C++/CX)](tracking-reference-operator-cpp-component-extensions.md)
## [Handle für Objekt-Operator (^) (C++/CLI und C++/CX)](handle-to-object-operator-hat-cpp-component-extensions.md)
## [Abstrakt (C++/CLI und C++/CX)](abstract-cpp-component-extensions.md)
## [Arrays (C++/CLI und C++/CX)](arrays-cpp-component-extensions.md)
## [Boxing (C++/CLI und C++/CX)](boxing-cpp-component-extensions.md)
## [Klassen und Strukturen (C++/CLI und C++/CX)](classes-and-structs-cpp-component-extensions.md)
## [Plattform-, Standard- und CLI-Namespaces (C++/CLI und C++/CX)](platform-default-and-cli-namespaces-cpp-component-extensions.md)
## [Compilerunterstützung für Typmerkmale (C++/CLI und C++/CX)](compiler-support-for-type-traits-cpp-component-extensions.md)
## [Kontextbezogene Schlüsselwörter (C++/CLI und C++/CX)](context-sensitive-keywords-cpp-component-extensions.md)
## [Delegat (C++/CLI und C++/CX)](delegate-cpp-component-extensions.md)
## [enum-Klasse (C++/CLI und C++/CX)](enum-class-cpp-component-extensions.md)
## [Ereignis (C++/CLI und C++/CX)](event-cpp-component-extensions.md)
## [Ausnahmebehandlung (C++/CLI und C++/CX)](exception-handling-cpp-component-extensions.md)
## [Explizite Außerkraftsetzungen (C++/CLI und C++/CX)](explicit-overrides-cpp-component-extensions.md)
## [ref new, gcnew (C++/CLI und C++/CX)](ref-new-gcnew-cpp-component-extensions.md)
## [Generika (C++/CLI und C++/CX)](generics-cpp-component-extensions.md)
### [Übersicht über Generika in Visual C++](overview-of-generics-in-visual-cpp.md)
### [Generische Funktionen (C++/CLI)](generic-functions-cpp-cli.md)
### [Generische Klassen (C++/CLI)](generic-classes-cpp-cli.md)
### [Generische Schnittstellen (C++/CLI)](generic-interfaces-visual-cpp.md)
### [Generische Delegaten (C++/CLI)](generic-delegates-visual-cpp.md)
### [Einschränkungen für generische Typparameter (C++/CLI)](constraints-on-generic-type-parameters-cpp-cli.md)
### [Verwenden von Generika (C++/CLI)](consuming-generics-cpp-cli.md)
### [Generika und Vorlagen (C++/CLI)](generics-and-templates-visual-cpp.md)
### [Vorgehensweise: Verbessern der Leistung mit Generika (C++/CLI)](how-to-improve-performance-with-generics-visual-cpp.md)
## [interface-Klasse (C++/CLI und C++/CX)](interface-class-cpp-component-extensions.md)
## [literal (C++/CLI und C++/CX)](literal-cpp-component-extensions.md)
## [Windows-Runtime und verwaltete Vorlagen (C++/CLI und C++/CX)](windows-runtime-and-managed-templates-cpp-component-extensions.md)
## [new (neuer Slot in vtable) (C++/CLI und C++/CX)](new-new-slot-in-vtable-cpp-component-extensions.md)
## [nullptr (C++/CLI und C++/CX)](nullptr-cpp-component-extensions.md)
## [Überschreibungsspezifizierer (C++/CLI und C++/CX)](override-specifiers-cpp-component-extensions.md)
## [override (C++/CLI und C++/CX)](override-cpp-component-extensions.md)
## [partial (C++/CLI und C++/CX)](partial-cpp-component-extensions.md)
## [property (C++/CLI und C++/CX)](property-cpp-component-extensions.md)
## [safe_cast (C++/CLI und C++/CX)](safe-cast-cpp-component-extensions.md)
## [String (Zeichenfolge) (C++/CLI und C++/CX)](string-cpp-component-extensions.md)
## [sealed (C++/CLI und C++/CX)](sealed-cpp-component-extensions.md)
## [typeid (C++/CLI und C++/CX)](typeid-cpp-component-extensions.md)
## [Benutzerdefinierte Attribute (C++/CLI und C++/CX)](user-defined-attributes-cpp-component-extensions.md)
### [Attributparametertypen (C++/CLI und C++/CX)](attribute-parameter-types-cpp-component-extensions.md)
### [Attributziele (C++/CLI und C++/CX)](attribute-targets-cpp-component-extensions.md)
## [Für C++/CLI spezifische Erweiterungen](extensions-that-are-specific-to-cpp-cli.md)
### [__identifier (C++/CLI)](identifier-cpp-cli.md)
### [C-stilartige Umwandlungen mit -clr (C++/CLI)](c-style-casts-with-clr-cpp-cli.md)
### [interior_ptr (C++/CLI)](interior-ptr-cpp-cli.md)
#### [Vorgehensweise: Deklarieren und Verwenden von inneren Zeigern und verwalteten Arrays (C++/CLI)](how-to-declare-and-use-interior-pointers-and-managed-arrays-cpp-cli.md)
#### [Vorgehensweise: Deklarieren von Werttypen mit dem interior_ptr-Schlüsselwort (C++/CLI)](how-to-declare-value-types-with-the-interior-ptr-keyword-cpp-cli.md)
#### [Vorgehensweise: Überladen von Funktionen mit inneren und nativen Zeigern (C++/CLI)](how-to-overload-functions-with-interior-pointers-and-native-pointers-cpp-cli.md)
#### [Vorgehensweise: Deklarieren von inneren Zeigern mit dem const-Schlüsselwort (C++/CLI)](how-to-declare-interior-pointers-with-the-const-keyword-cpp-cli.md)
### [pin_ptr (C++/CLI)](pin-ptr-cpp-cli.md)
#### [Vorgehensweise: Anheften von Zeigern und Arrays](how-to-pin-pointers-and-arrays.md)
#### [Vorgehensweise: Deklarieren von festen Zeigern und Werttypen](how-to-declare-pinning-pointers-and-value-types.md)
### [Typweiterleitung (C++/CLI)](type-forwarding-cpp-cli.md)
### [Variable Argumentlisten (...) (C++/CLI)](variable-argument-lists-dot-dot-dot-cpp-cli.md)
## [Ressourcen zum Erstellen eines Spiels mit DirectX](resources-for-creating-a-game-using-directx.md)
## [Exemplarische Vorgehensweise: Erstellen und Verwenden einer statischen Bibliothek (C++)](walkthrough-creating-and-using-a-static-library-cpp.md)
## [Vorgehensweise: Verwenden des Windows 10 SDK in einer Windows-Desktopanwendung](how-to-use-the-windows-10-sdk-in-a-windows-desktop-application.md)
# [Universelle Windows-Apps (C++)](universal-windows-apps-cpp.md)
## [C++/CX](../cppcx/TOC.md)
## [C++-Vorlagenbibliothek für Windows-Runtime (WRL)](wrl/windows-runtime-cpp-template-library-wrl.md)
### [Vorgehensweise: Aktivieren und Verwenden einer Windows-Runtime-Komponente mit WRL](wrl/how-to-activate-and-use-a-windows-runtime-component-using-wrl.md)
### [Vorgehensweise: Abschließen asynchroner Vorgänge mit WRL](wrl/how-to-complete-asynchronous-operations-using-wrl.md)
### [Vorgehensweise: Behandeln von Ereignissen mit WRL](wrl/how-to-handle-events-using-wrl.md)
### [Exemplarische Vorgehensweise: Erstellen einer UWP-App mithilfe von WRL und Media Foundation](wrl/walkthrough-creating-a-windows-store-app-using-wrl-and-media-foundation.md)
### [Vorgehensweise: Erstellen einer klassischen COM-Komponente mit WRL](wrl/how-to-create-a-classic-com-component-using-wrl.md)
### [Vorgehensweise: Direktes Instanziieren von WRL-Komponenten](wrl/how-to-instantiate-wrl-components-directly.md)
### [Vorgehensweise: Verwenden von „winmdidl.exe“ und „midlrt.exe“ zum Erstellen von .h-Dateien aus Windows-Metadaten](wrl/use-winmdidl-and-midlrt-to-create-h-files-from-windows-metadata.md)
### [Schlüssel-WRL-APIs nach Kategorie](wrl/key-wrl-apis-by-category.md)
### [WRL-Referenz](wrl/wrl-reference.md)
#### [Microsoft::WRL-Namespace](wrl/microsoft-wrl-namespace.md)
##### [ActivatableClass-Makros](wrl/activatableclass-macros.md)
##### [ActivationFactory-Klasse](wrl/activationfactory-class.md)
##### [AgileActivationFactory-Klasse](wrl/agileactivationfactory-class.md)
##### [AgileEventSource-Klasse](wrl/agileeventsource-class.md)
##### [AsWeak-Funktion](wrl/asweak-function.md)
##### [AsyncBase-Klasse](wrl/asyncbase-class.md)
##### [AsyncResultType-Enumeration](wrl/asyncresulttype-enumeration.md)
##### [Rückruffunktion (WRL)](wrl/callback-function-wrl.md)
##### [CancelTransitionPolicy-Enumeration](wrl/canceltransitionpolicy-enumeration.md)
##### [ChainInterfaces-Struktur](wrl/chaininterfaces-structure.md)
##### [ClassFactory-Klasse](wrl/classfactory-class.md)
##### [CloakedIid-Struktur](wrl/cloakediid-structure.md)
##### [ComposableBase-Klasse](wrl/composablebase-class.md)
##### [ComPtr-Klasse](wrl/comptr-class.md)
##### [CreateActivationFactory-Funktion](wrl/createactivationfactory-function.md)
##### [CreateClassFactory-Funktion](wrl/createclassfactory-function.md)
##### [DeferrableEventArgs-Klasse](wrl/deferrableeventargs-class.md)
##### [EventSource-Klasse](wrl/eventsource-class.md)
##### [FactoryCacheFlags-Enumeration](wrl/factorycacheflags-enumeration.md)
##### [FtmBase-Klasse](wrl/ftmbase-class.md)
##### [GetModuleBase-Funktion](wrl/getmodulebase-function.md)
##### [Implements-Struktur](wrl/implements-structure.md)
##### [InspectableClass-Makro](wrl/inspectableclass-macro.md)
##### [InvokeModeOptions-Struktur](wrl/invokemodeoptions-structure.md)
##### [Make-Funktion](wrl/make-function.md)
##### [MixIn-Struktur](wrl/mixin-structure.md)
##### [Module-Klasse](wrl/module-class.md)
###### [Module::GenericReleaseNotifier-Klasse](wrl/module-genericreleasenotifier-class.md)
###### [Module::MethodReleaseNotifier-Klasse](wrl/module-methodreleasenotifier-class.md)
###### [Module::ReleaseNotifier-Klasse](wrl/module-releasenotifier-class.md)
##### [ModuleType-Enumeration](wrl/moduletype-enumeration.md)
##### [operator!=-Operator (Microsoft::WRL)](wrl/operator-inequality-operator-microsoft-wrl.md)
##### [operator==-Operator (Microsoft::WRL)](wrl/operator-equality-operator-microsoft-wrl.md)
##### [operator<-Operator (Microsoft::WRL)](wrl/operator-less-than-operator-microsoft-wrl.md)
##### [RuntimeClass-Klasse](wrl/runtimeclass-class.md)
##### [RuntimeClassFlags-Struktur](wrl/runtimeclassflags-structure.md)
##### [RuntimeClassType-Enumeration](wrl/runtimeclasstype-enumeration.md)
##### [SimpleActivationFactory-Klasse](wrl/simpleactivationfactory-class.md)
##### [SimpleClassFactory-Klasse](wrl/simpleclassfactory-class.md)
##### [WeakRef-Klasse](wrl/weakref-class.md)
#### [Microsoft::WRL::Details-Namespace](wrl/microsoft-wrl-details-namespace.md)
##### [ActivationFactoryCallback-Funktion](wrl/activationfactorycallback-function.md)
##### [ArgTraits-Struktur](wrl/argtraits-structure.md)
##### [ArgTraitsHelper-Struktur](wrl/argtraitshelper-structure.md)
##### [AsyncStatusInternal-Enumeration](wrl/asyncstatusinternal-enumeration.md)
##### [BoolStruct-Struktur](wrl/boolstruct-structure.md)
##### [ComPtrRef-Klasse](wrl/comptrref-class.md)
##### [ComPtrRefBase-Klasse](wrl/comptrrefbase-class.md)
##### [CreatorMap-Struktur](wrl/creatormap-structure.md)
##### [DerefHelper-Struktur](wrl/derefhelper-structure.md)
##### [DontUseNewUseMake-Klasse](wrl/dontusenewusemake-class.md)
##### [EnableIf-Struktur](wrl/enableif-structure.md)
##### [EventTargetArray-Klasse](wrl/eventtargetarray-class.md)
##### [FactoryCache-Struktur](wrl/factorycache-structure.md)
##### [ImplementsBase-Struktur](wrl/implementsbase-structure.md)
##### [ImplementsHelper-Struktur](wrl/implementshelper-structure.md)
##### [InterfaceList-Struktur](wrl/interfacelist-structure.md)
##### [InterfaceListHelper-Struktur](wrl/interfacelisthelper-structure.md)
##### [InterfaceTraits-Struktur](wrl/interfacetraits-structure.md)
##### [InvokeHelper-Struktur](wrl/invokehelper-structure.md)
##### [IsBaseOfStrict-Struktur](wrl/isbaseofstrict-structure.md)
##### [IsSame-Struktur](wrl/issame-structure.md)
##### [MakeAllocator-Klasse](wrl/makeallocator-class.md)
##### [MakeAndInitialize-Funktion](wrl/makeandinitialize-function.md)
##### [ModuleBase-Klasse](wrl/modulebase-class.md)
##### [Move-Funktion](wrl/move-function.md)
##### [Nil-Struktur](wrl/nil-structure.md)
##### [RaiseException-Funktion](wrl/raiseexception-function.md)
##### [RemoveIUnknown-Klasse](wrl/removeiunknown-class.md)
##### [RemoveReference-Struktur](wrl/removereference-structure.md)
##### [RuntimeClassBase-Struktur](wrl/runtimeclassbase-structure.md)
##### [RuntimeClassBaseT-Struktur](wrl/runtimeclassbaset-structure.md)
##### [Swap-Funktion (WRL)](wrl/swap-function-wrl.md)
##### [TerminateMap-Funktion](wrl/terminatemap-function.md)
##### [VerifyInheritanceHelper-Struktur](wrl/verifyinheritancehelper-structure.md)
##### [VerifyInterfaceHelper-Struktur](wrl/verifyinterfacehelper-structure.md)
##### [WeakReference-Klasse](wrl/weakreference-class.md)
#### [Microsoft::WRL::Wrappers-Namespace](wrl/microsoft-wrl-wrappers-namespace.md)
##### [CriticalSection-Klasse](wrl/criticalsection-class.md)
##### [Ereignisklasse (WRL)](wrl/event-class-wrl.md)
##### [HandleT-Klasse](wrl/handlet-class.md)
##### [HString-Klasse](wrl/hstring-class.md)
##### [HStringReference-Klasse](wrl/hstringreference-class.md)
##### [Mutex-Klasse](wrl/mutex-class.md)
##### [RoInitializeWrapper-Klasse](wrl/roinitializewrapper-class.md)
##### [Semaphore-Klasse](wrl/semaphore-class.md)
##### [SRWLock-Klasse](wrl/srwlock-class.md)
#### [Microsoft::WRL::Wrappers::Details-Namespace](wrl/microsoft-wrl-wrappers-details-namespace.md)
##### [CompareStringOrdinal-Methode](wrl/comparestringordinal-method.md)
##### [SyncLockT-Klasse](wrl/synclockt-class.md)
##### [SyncLockWithStatusT-Klasse](wrl/synclockwithstatust-class.md)
#### [Microsoft::WRL::Wrappers::HandleTraits-Namespace](wrl/microsoft-wrl-wrappers-handletraits-namespace.md)
##### [CriticalSectionTraits-Struktur](wrl/criticalsectiontraits-structure.md)
##### [EventTraits-Struktur](wrl/eventtraits-structure.md)
##### [FileHandleTraits-Struktur](wrl/filehandletraits-structure.md)
##### [HANDLENullTraits-Struktur](wrl/handlenulltraits-structure.md)
##### [HANDLETraits-Struktur](wrl/handletraits-structure.md)
##### [MutexTraits-Struktur](wrl/mutextraits-structure.md)
##### [SemaphoreTraits-Struktur](wrl/semaphoretraits-structure.md)
##### [SRWLockExclusiveTraits-Struktur](wrl/srwlockexclusivetraits-structure.md)
##### [SRWLockSharedTraits-Struktur](wrl/srwlocksharedtraits-structure.md)
#### [Windows::Foundation-Namespace](wrl/windows-foundation-namespace.md)
##### [ActivateInstance-Funktion](wrl/activateinstance-function.md)
##### [GetActivationFactory-Funktion](wrl/getactivationfactory-function.md)
#### [IID_PPV_ARGS_Helper-Funktion](wrl/iid-ppv-args-helper-function.md)
# [SafeInt-Bibliothek](safeint-library.md)
## [SafeInt-Klasse](safeint-class.md)
## [SafeInt-Funktionen](safeint-functions.md)
## [SafeIntException-Klasse](safeintexception-class.md)
# [C++-Attribute für COM und .NET](attributes/cpp-attributes-com-net.md)
## [Fragen und Antworten zur attributierten Programmierung](attributes/attribute-programming-faq.md)
## [Attribute nach Gruppen](attributes/attributes-by-group.md)
### [COM-Attribute](attributes/com-attributes.md)
### [IDL-Attribute](attributes/idl-attributes.md)
### [OLE DB-Consumerattribute](attributes/ole-db-consumer-attributes.md)
### [Compilerattribute](attributes/compiler-attributes.md)
## [Attribute nach Verwendung](attributes/attributes-by-usage.md)
### [Modulattribute](attributes/module-attributes.md)
### [Schnittstellenattribut](attributes/interface-attributes.md)
### [Klassenattribute](attributes/class-attributes.md)
### [Methodenattribut](attributes/method-attributes.md)
### [Parameterattribute](attributes/parameter-attributes.md)
### [Datenmemberattribute](attributes/data-member-attributes.md)
### [typedef-, enum-, union- und struct-Attribute](attributes/typedef-enum-union-and-struct-attributes.md)
### [Arrayattribute](attributes/array-attributes.md)
### [Eigenständige Attribute](attributes/stand-alone-attributes.md)
### [Benutzerdefinierte Attribute](attributes/custom-attributes-cpp.md)
## [Alphabetische Attributreferenz](attributes/attributes-alphabetical-reference.md)
### [Aggregierbar](attributes/aggregatable.md)
### [Aggregate](attributes/aggregates.md)
### [appobject](attributes/appobject.md)
### [async_uuid](attributes/async-uuid.md)
### [Attribut](attributes/attribute.md)
### [bindable](attributes/bindable.md)
### [call_as](attributes/call-as.md)
### [case](attributes/case-cpp.md)
### [coclass](attributes/coclass.md)
### [com_interface_entry](attributes/com-interface-entry-cpp.md)
### [Steuerelement](attributes/control.md)
### [cpp_quote](attributes/cpp-quote.md)
### [custom](attributes/custom-cpp.md)
### [db_accessor](attributes/db-accessor.md)
### [db_column](attributes/db-column.md)
### [db_command](attributes/db-command.md)
### [db_param](attributes/db-param.md)
### [db_source](attributes/db-source.md)
### [db_table](attributes/db-table.md)
### [default](attributes/default-cpp.md)
### [defaultbind](attributes/defaultbind.md)
### [defaultcollelem](attributes/defaultcollelem.md)
### [defaultvalue](attributes/defaultvalue.md)
### [defaultvtable](attributes/defaultvtable.md)
### [dispinterface](attributes/dispinterface.md)
### [displaybind](attributes/displaybind.md)
### [dual](attributes/dual.md)
### [emitidl](attributes/emitidl.md)
### [entry](attributes/entry.md)
### [event_receiver](attributes/event-receiver.md)
### [event_source](attributes/event-source.md)
### [export](attributes/export.md)
### [first_is](attributes/first-is.md)
### [helpcontext](attributes/helpcontext.md)
### [helpfile](attributes/helpfile.md)
### [helpstring](attributes/helpstring.md)
### [helpstringcontext](attributes/helpstringcontext.md)
### [helpstringdll](attributes/helpstringdll.md)
### [hidden](attributes/hidden.md)
### [id](attributes/id.md)
### [idl_module](attributes/idl-module.md)
### [idl_quote](attributes/idl-quote.md)
### [iid_is](attributes/iid-is.md)
### [immediatebind](attributes/immediatebind.md)
### [implements](attributes/implements-cpp.md)
### [implements_category](attributes/implements-category.md)
### [import](attributes/import.md)
### [importidl](attributes/importidl.md)
### [importlib](attributes/importlib.md)
### [in](attributes/in-cpp.md)
### [include](attributes/include-cpp.md)
### [includelib](attributes/includelib-cpp.md)
### [last_is](attributes/last-is.md)
### [lcid](attributes/lcid.md)
### [length_is](attributes/length-is.md)
### [library_block](attributes/library-block.md)
### [licensed](attributes/licensed.md)
### [local](attributes/local-cpp.md)
### [max_is](attributes/max-is.md)
### [module](attributes/module-cpp.md)
### [ms_union](attributes/ms-union.md)
### [no_injected_text](attributes/no-injected-text.md)
### [nonbrowsable](attributes/nonbrowsable.md)
### [noncreatable](attributes/noncreatable.md)
### [nonextensible](attributes/nonextensible.md)
### [object](attributes/object-cpp.md)
### [odl](attributes/odl.md)
### [oleautomation](attributes/oleautomation.md)
### [optional](attributes/optional-cpp.md)
### [out](attributes/out-cpp.md)
### [pointer_default](attributes/pointer-default.md)
### [pragma](attributes/pragma.md)
### [progid](attributes/progid.md)
### [propget](attributes/propget.md)
### [propput](attributes/propput.md)
### [propputref](attributes/propputref.md)
### [ptr](attributes/ptr.md)
### [public (C++-Attribute)](attributes/public-cpp-attributes.md)
### [range](attributes/range-cpp.md)
### [rdx](attributes/rdx.md)
### [readonly](attributes/readonly-cpp.md)
### [ref](attributes/ref-cpp.md)
### [registration_script](attributes/registration-script.md)
### [requestedit](attributes/requestedit.md)
### [requires_category](attributes/requires-category.md)
### [restricted](attributes/restricted.md)
### [retval](attributes/retval.md)
### [satype](attributes/satype.md)
### [size_is](attributes/size-is.md)
### [source](attributes/source-cpp.md)
### [string](attributes/string-cpp.md)
### [support_error_info](attributes/support-error-info.md)
### [switch_is](attributes/switch-is.md)
### [switch_type](attributes/switch-type.md)
### [synchronize](attributes/synchronize.md)
### [threading](attributes/threading-cpp.md)
### [transmit_as](attributes/transmit-as.md)
### [uidefault](attributes/uidefault.md)
### [unique](attributes/unique-cpp.md)
### [usesgetlasterror](attributes/usesgetlasterror.md)
### [uuid (C++-Attribute)](attributes/uuid-cpp-attributes.md)
### [v1_enum](attributes/v1-enum.md)
### [vararg](attributes/vararg.md)
### [version](attributes/version-cpp.md)
### [vi_progid](attributes/vi-progid.md)
### [wire_marshal](attributes/wire-marshal.md)
