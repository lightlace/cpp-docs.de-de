...
- Wird `RaiseException` aufgerufen, wenn ein Problem erkannt wird.
...
|----------|--------------------------------|-----------------|
|31-30|11|Diese beiden Bits beschreiben den grundlegenden Status des Codes:  11 = Fehler, 00 = Erfolg, 01 = Information, 10 = Warnung.|
...
Nachdem Sie einen Ausnahmecode definiert haben, können Sie ihn verwenden, um eine Ausnahme auszulösen. Beispielsweise löst der folgende Code die Ausnahme `STATUS_INSUFFICIENT_MEM` als Reaktion auf ein Problem mit der Speicher Belegung aus:
