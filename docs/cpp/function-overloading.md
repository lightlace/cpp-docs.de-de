...
dev_langs: 
  - "C++"
helpviewer_keywords: 
  - "Deklarieren von Funktionen, Überladen"
  - "Funktionsüberladung"
...
|**const** oder `volatile` \(siehe unten\)|Ja|  
 ...
 Obwohl Funktionen auf Grundlage des Rückgabetyps unterschieden werden können, können sie nicht auf dieser Grundlage überladen werden.  `Const` oder `volatile` dienen nur als Grundlage zum Überladen, wenn sie in einer Klasse verwendet werden, die auf den **this**\-Zeiger für die Klasse und nicht auf den Rückgabetyp der Funktion angewendet werden soll.  Das heißt, Überladung gilt nur, wenn das Schlüsselwort **const** oder `volatile` der Argumentliste der Funktion in der Deklaration folgt.  
...
```  
 ...
 Weitere Einschränkungen beim Überladen und Informationen dazu, wie sich das Überladen auf andere Elemente von C\+\+ auswirkt, finden Sie unter [Überladen](../misc/overloading-cpp.md).  
  
## Argumentübereinstimmung  
 Überladene Funktionen werden für die beste Übereinstimmung von Funktionsdeklarationen im aktuellen Bereich für Argumente ausgewählt, die im Funktionsaufruf angegeben werden.  Wenn eine passende Funktion gefunden wird, wird diese Funktion in diesem Kontext als „  Passend“ bezeichnet, was Folgendes bedeutet:  
...
## Unterschiede bei Argumenttypen  
 ...
 Aus demselben Grund werden Funktionsargumente eines Typs, der durch **const** oder `volatile` geändert wird, nicht anders behandelt als der Basistyp für den Zweck des Überladens.  
  
 Allerdings kann der Funktionsüberladungsmechanismus zwischen Verweisen, die von **const** sowie `volatile` qualifiziert sind, und Verweisen auf den Basistyp unterscheiden.  Dies ermöglicht Code wie den folgenden:  
...
```  
 ...
 Zeiger auf **const**\- und `volatile`\-Objekte gelten auch als Zeiger, die sich von Zeigern auf den Basistyp zum Zweck des Überladens unterscheiden.  
...
-   Sequenzen von Konvertierungen, die gekürzt werden können, indem Konvertierungen im Zwischenformat entfernt werden, werden nicht berücksichtigt.  
 ...
 Das Ergebnis der Konvertierungssequenz, falls vorhanden, ist die am besten passende Sequenz.  Ein Objekt vom Typ `int` kann auf unterschiedliche Weise mit Standardkonvertierungen \(wie unter [Standardwertkonvertierungen](../cpp/standard-conversions.md) beschrieben\) in den Typ `unsigned long` konvertiert werden:  
...
|*Typname*|**const** *type\-name*|  
|*Typname*|`volatile` *type\-name*|  
...
1.  Genaue Übereinstimmung.  Ein genaue Übereinstimmung zwischen den Typen, mit denen die Funktion aufgerufen wird und den Typen, die im Funktionsprototyp deklariert werden, ist immer die beste Übereinstimmung.  Sequenzen von trivialen Konvertierungen werden als exakte Übereinstimmungen klassifiziert.  Allerdings sind Sequenzen, die keine dieser Konvertierungen ausführen, besser als Sequenzen, die konvertieren:  
    ...
    -   Von Zeiger in Zeiger auf **const** \(`type` **\*** in **const** `type` **\***\).  
  
    -   Von Zeiger in Zeiger auf `volatile` \(`type` **\*** in `volatile` `type` **\***\).  
  
    -   Von Verweis in Verweis auf **const** \(`type` **&** in **const** `type` **&**\).  
  
    -   Von Verweis in Verweis auf `volatile` \(`type` **&** in `volatile` `type` **&**\).  
  
2.  Übereinstimmung mithilfe von Erweiterungen.  Jede beliebige Sequenz, die nicht als genaue Übereinstimmung klassifiziert wird und die nur ganzzahlige Erweiterungen, Konvertierungen von **float** in **double** und triviale Konvertierungen enthält, wird als Übereinstimmung mithilfe von Erweiterungen klassifiziert.  Obwohl eine Übereinstimmung mit Erweiterungen besser ist als eine Übereinstimmung, die Standardkonvertierungen verwendet, ist sie nicht so gut wie eine genaue Übereinstimmung.  
...
3.  Übereinstimmung mithilfe von Standardkonvertierungen.  Jede beliebige Sequenz, die nicht als genaue Übereinstimmung klassifiziert wird, oder eine Übereinstimmung mithilfe von Erweiterungen, die nur Standardkonvertierungen und triviale Konvertierungen enthält, wird als Übereinstimmung mithilfe von Standardkonvertierungen klassifiziert.  In dieser Kategorie gelten die folgenden Regeln:  
    ...
    -   Die Konvertierung von einem Zeiger auf eine abgeleitete Klasse in einen Zeiger auf eine direkte oder indirekte Basisklasse ist einer Konvertierung in **void \*** oder **const void \*** vorzuziehen.  
 ...
 ![Bevorzugte Konvertierungen](../cpp/media/vc391t1.png "vc391T1")  
Diagramm zur Veranschaulichung bevorzugter Konvertierungen  
 ...
 ![Mehrfache Vererbung mit bevorzugten Konvertierungen](../cpp/media/vc391t2.png "vc391T2")  
...
```  
 ...
 Die verfügbaren benutzerdefinierten Konvertierungen für die `UDC`\-Klasse sind vom Typ `int` und **long**.  Deshalb berücksichtigt der Compiler Konvertierungen für den Typ des Objekts, das abgeglichen wird: `UDC`.  Eine Konvertierung in `int` ist vorhanden, und sie ist ausgewählt.  
...
```  
 ...
 Im vorherigen Beispiel wird die benutzerdefinierte Konvertierung **operator long** aufgerufen, um `udc` in den Typ **long** zu konvertieren.  Wenn keine benutzerdefinierte Konvertierung für den Typ **long** definiert ist, wird die Konvertierung wie folgt fortgesetzt: Der Typ `UDC` wird mittels benutzerdefinierter Konvertierung in den Typ `int` umgewandelt.  Anschließend wird die Standardkonvertierung vom Typ `int` in den Typ **long** entsprechend dem Argument in der Deklaration angewendet.  
...
-   Konvertieren vom Typ `int` in den Typ **long**; anschließende Konvertierung in den Typ `UDC2` \(Zwei\-Schritte\-Konvertierung\).  
