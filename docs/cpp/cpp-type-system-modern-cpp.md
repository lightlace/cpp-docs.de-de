...
![Größe in Byte der integrierten&#45;Typen](../cpp/media/built-intypesizes.png "Größe in Byte integrierter&#45;Typen")
...
|Ganzzahl ohne Vorzeichen&nbsp;Char|1 Byte|C++ verfügt über keine integrierten `byte`-Typen.  Verwenden Sie "char" ohne Vorzeichen, um einen Bytewert darzustellen.|
...
Der **void** -Typ ist ein spezieller Typ. Sie können eine Variable vom Typ " **void**" nicht deklarieren, aber Sie können eine Variable vom Typ " __void \*__  " (Zeiger auf " **void**") deklarieren, was manchmal notwendig ist, wenn Sie unformatierten (nicht typisierten) Speicher belegen Zeiger auf " **void** " sind jedoch nicht typsicher, und in der Regel wird in der modernen C++Verwendung dringend davon abgeraten. In einer Funktionsdeklaration bedeutet der Rückgabewert **void** , dass die Funktion keinen Wert zurückgibt. Dies ist eine gängige und akzeptable Verwendung von **void**. Obwohl in der Programmiersprache C Funktionen erforderlich sind, die NULL-Parameter aufweisen, um **void** in der Parameter `fou(void)`Liste zu deklarieren (z. b.), `fou()`wird diese Vorgehensweise in modernen C++ und sollte deklariert werden. Weitere Informationen finden Sie unter [Typkonvertierungen und Typsicherheit](../cpp/type-conversions-and-type-safety-modern-cpp.md).
...
Streng genommen hat die C++ Sprache keinen integrierten Zeichen foldetyp. Zeichen für **char** -und **wchar_t** -Speicher: Sie müssen ein Array dieser Typen deklarieren, um eine Zeichenfolge zu finden, und dem Array Element einen `'\0'`NULL-Beendigungs Wert (z. b. ASCII) hinzufügen, der hinter dem letzten gültigen Zeichen (auch als *Zeichenfolge im C-Stil*). Für Zeichenfolgen im C-Stil ist das Schreiben von viel mehr Code oder die Verwendung externer Hilfsprogrammbibliotheken für Zeichenfolgefunktionen erforderlich. In modernen C++haben wir jedoch die Standard Bibliotheks `std::string` Typen (für Zeichen folgen mit 8 **-Bit-** Zeichen folgen) oder `std::wstring` (für Zeichen folgen mit 16-Bit-Zeichen folgen vom Typ " **wchar_t**"). Diese C++ Standard Bibliotheks Container können als systemeigene Zeichen folgen Typen betrachtet werden, da Sie Teil der Standardbibliotheken sind, die in einer beliebigen C++ kompatiblen Buildumgebung enthalten sind. Verwenden Sie einfach die `#include <string>`-Anweisung, um diese Typen im Programm bereitzustellen. (Wenn Sie MFC oder ATL verwenden, ist auch die CString-Klasse verfügbar, aber Sie ist nicht Bestandteil des C++-Standards.) Von der Verwendung von auf NULL endenden Zeichenarrays (die bereits erwähnten Zeichenfolgen im C-Stil) wird im modernen C++ Abstand genommen.
...
Im korrigierten Codebeispiel wird lokaler Stapelarbeitsspeicher zum Erstellen von Sicherungsspeicher, auf den `pNumber` verweist, verwendet. Wir verwenden der Einfachheit halber einen grundlegenden Typ. In der Praxis handelt es sich bei dem Sicherungs Speicher für Zeiger meistens um benutzerdefinierte Typen, die in einem Bereich des Arbeitsspeichers, der als *Heap* (oder als *freier*Speicher bezeichnet) bezeichnet wird, mithilfe eines **neuen** Schlüsselwort Ausdrucks dynamisch zugeordnet werden (im C-Stil, der ältere `malloc()` Die C-Lauf Zeit Bibliotheksfunktion wurde verwendet.) Nach der Zuordnung werden diese Variablen normalerweise als Objekte bezeichnet, insbesondere, wenn Sie auf einer Klassendefinition basieren. Arbeitsspeicher, der mit **New** zugewiesen wird, muss durch eine entsprechende **Delete** -Anweisung gelöscht werden (oder, `malloc()` Wenn Sie die Funktion verwendet haben, um Sie `free()`zuzuweisen, die C-Lauf Zeitfunktion).
